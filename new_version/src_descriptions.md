...
- `print_symbolic_decision(symbolic, max_chars=240)`: prints the decision function (truncated to ``max_chars`` for Jupyter responsiveness; pass ``max_chars=None`` for the full expression), a one-line reference to the P(up) formula, and the surviving features. Long sympy expressions (~1k+ characters on typical extraction folds) take noticeable wall time for Jupyter to render due to syntax highlighting and word-wrap calculations, not because the underlying Python `print()` is slow; the default truncation keeps the cell snappy and the full string remains available via ``symbolic['decision_function']``.
- `print_extraction_metrics(symbolic)`: prints pre/post symbolic accuracy, symbolification rate, and the pruned architecture. The accuracy gap (pre minus post) measures the cost of replacing spline activations with closed-form symbolic primitives. This helper is itself a fixed-size readout of pre-computed floats; the perceived slowness users sometimes attribute to it is actually the render time of the long decision-function string from the preceding `print_symbolic_decision` call landing late in the cell output.
