...
The final per-feature MDA is the average across both classifiers and all inner folds. Features with positive MDA are kept (permuting them hurts at least one model on average), capped at the top 20% by MDA value (the locked configuration), with a hard floor of 5 features minimum. The cap was tightened in two stages from the common 0.40-0.50 default. The first tightening, from 0.40 to 0.30, came after a high-PBO run revealed only ~6 features clearing 50% selection frequency in the stability bar chart, indicating that the long tail was contributing variance rather than signal. At that 66-feature pool size, 0.25 vs 0.30 was evaluated as a sensitivity pair: both leave predictive metrics (mean accuracy, pooled AUC) essentially unchanged across all six models, but path-Sharpe-derived metrics (Sharpe, DSR, PBO) diverge substantially. 0.30 was selected as primary because it produced lower PBO (0.26 vs 0.69), indicating more robust model selection under the AFML interpretation. The second tightening, from 0.30 to 0.20, accompanied the May 2026 expansion of the feature pool from 66 to 73 columns (the 14d/30d macro pairs and the AR_LAGS extension). Holding 0.30 against the larger pool would let ~22 features pass per fold, re-introducing the long-tail variance the original tightening had addressed; 0.20 × 73 ≈ 15 features per fold restores the absolute count to roughly the pre-expansion working point. The locked configuration is therefore `MDA_TOP_K_FRAC = 0.20`. The earlier-iteration findings (0.25 vs 0.30 sensitivity at the 66-feature pool size) are preserved in the methodology trail; the divergence between cap settings on backtest metrics despite stable predictive metrics is itself the rank-instability finding manifest on this dataset.
