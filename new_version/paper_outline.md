...
- **Selection rule.** Keep features with averaged MDA > 0; cap at `MDA_TOP_K_FRAC = 0.30` (approximately 18 of 62); minimum floor of 5 features.
- **TOP_K_FRAC tightening rationale (advisor-reviewed).** The cap was tightened from an earlier 0.40 to 0.30 in the locked configuration after a high-PBO run with the looser setting. Across the earlier-run CPCV folds, only approximately 6 features cleared 50% selection frequency in the stability bar chart, indicating that the long tail of the MDA-ranked feature set was contributing variance rather than signal. Tightening to 0.30 forces approximately 18 features through the bottleneck and aligns the selection cap with the empirical stability finding. The trade-off is that one or two folds may select fewer features than they would have at 0.40, but those folds were also the ones contributing the most rank variance to PBO, so the tightening attacks the right problem. An even tighter 0.25 cap (approximately 15 features) was tested in an interim configuration; 0.30 was retained as the final value because it preserves enough feature diversity in the inner Optuna step without re-introducing the long tail.
- **AR Logistic exception.** Bypasses MDA entirely; receives pre-MDA matrix and selects 6 lag columns by name.
- Typical result: approximately 18 features selected per fold from 62 candidates (down from approximately 22 to 26 under the previous 0.40 cap; up from approximately 15 under the interim 0.25 cap).
...
- "PBO = 0.2571 lands in the robust-selection regime: in only 9 of 35 IS/OOS partitions does the in-sample best model underperform the out-of-sample median. Model rankings are reliable across sub-path partitions, even though no individual ranking is statistically significant. Leave-one-out PBO sharpens the picture: excluding any one of the four nonlinear models (Random Forest, XGBoost, LSTM, KAN) collapses PBO to 0.000, while excluding either of the two linear baselines (AR Logistic, Logistic Regression) leaves PBO unchanged at 0.257. The IS/OOS instability is therefore localised in the nonlinear-vs-linear contest, not in any one model individually."
...
- **Key observations (state facts here, save interpretation for 5.1).**
  ...
  - **Std Sharpe outlier.** AR Logistic shows the lowest std Sharpe (0.4725) by a factor of two compared to the other models. Pure momentum is consistent across paths because it conditions on a simple, stable signal; the cost is one of the lowest medians. LSTM has the highest std Sharpe (1.5063), and Random Forest, XGBoost, and KAN all sit in the 1.0 to 1.3 band; the leave-one-out PBO finding (Section 4.4) shows that any one of these four nonlinear models can absorb the IS/OOS instability, so LSTM is not uniquely responsible for the variance even though it carries the largest path-Sharpe std.
...
- **PBO result.** **PBO = 0.2571 = 9 of 35 IS/OOS partitions.** Interpretation in context of DSR < 0.95:
  ...
  - **Leave-one-out PBO sharpens the picture.** Re-running PBO on the 5×7 Sharpe matrix with one model dropped at a time produces a striking pattern: excluding any of the four nonlinear models (Random Forest, XGBoost, LSTM, KAN) collapses PBO to **0.000**; excluding either of the two linear baselines (AR Logistic, Logistic Regression) leaves PBO unchanged at **0.257**. Full table:
...
| logistic | 0.257 | +0.000 |
  ...
  - **Reading.** The IS/OOS instability is not localised in any single model. It is localised in the **competition among the four nonlinear models** for the IS-best slot. When all four are present, the IS-winner shuffles across nonlinear models in different sub-path partitions, producing the 9 of 35 failed IS/OOS partitions; remove any one of them and the remaining three nonlinear models settle into a stable IS ranking that the OOS median agrees with. The two linear baselines are too far behind in median Sharpe to ever be IS-winners, so removing them changes nothing. This is a more nuanced finding than "LSTM is the variance source": **LSTM, KAN, RF, and XGBoost are interchangeable from a PBO perspective**, and the non-zero six-model PBO of 0.2571 reflects competition among them rather than any one model's path-instability signature.
  - Joint reading with DSR < 0.95: model selection across the linear-vs-nonlinear contrast is reliable, but selection among the four nonlinear models is unreliable, and the model that any selection rule picks does not achieve a Sharpe that survives multiple-testing correction. The honest conclusion is "we know the four nonlinear models beat the two linear baselines, we cannot reliably rank within the nonlinear group, and none of the nonlinear models is statistically significant once selection bias is corrected for".
  - Methodological implication (T-R15: write for reviewers, this is the AFML pay-off). Without PBO, the natural conclusion from the model-comparison table is "LSTM achieved median Sharpe 1.42 on BTC daily direction". With the headline PBO of 0.2571, the conclusion sharpens: rankings are stable enough to claim that nonlinear models beat linear ones, but the absolute level of 1.42 sits below the multiple-testing-corrected significance threshold (DSR = 0.42, far below 0.95). The leave-one-out PBO refines the message further: the apparent ranking among the four nonlinear models is not reproducible across sub-path partitions, so "LSTM is the best model" is too strong a claim for what the data supports; "the four nonlinear models are statistically indistinguishable as a group, and that group beats the linear baselines" is the most defensible reading. The PBO and DSR diagnostics are complementary: PBO answers "did we pick the right model?", DSR answers "is the right model good enough?"; here the answer is "yes for the linear-vs-nonlinear distinction, no within the nonlinear group" to the first and "no" to the second.
...
- **Numerical sensitivity at the dataset mean.** Partial derivatives evaluated at the mean of each feature, scaled by the feature's standard deviation, give the per-σ effect on the decision function:
  ...
  - `exchange_supply_pct` (mean 13.83, std 2.39): σ-effect on the decision function -4.55. **The largest meaningful effect in the formula.** A one-standard-deviation increase in the percentage of BTC supply held on exchanges decreases the decision-function (logit) by approximately 4.55 units, which corresponds to moving P(up) from roughly 0.50 to roughly 0.01, a saturating rather than a linear effect. The linearised approximation σ Δp ≈ -1.14 (= σ-effect / 4 at p=0.5) is mechanically what the partial-derivative table reports, but it sits well outside the regime where the sigmoid linearisation is valid; the honest reading is that `exchange_supply_pct` drives the model from "neutral" to "strongly bearish" within a single σ bump. Sign aligns with conventional reading: more BTC on exchanges signals selling pressure, lowering forward direction probability.
  - `silver_ret_30` (mean 0.008, std 0.082): σ-effect -0.072, approximate σ Δp ≈ -0.018. Modest negative effect.
- **On-chain comment (Q2 evidence).** **One on-chain feature survives extraction** (`exchange_supply_pct`), and it is the surviving feature with the largest meaningful per-σ effect on the decision function (-4.55 in logit, equivalent to driving P(up) from approximately 0.50 to approximately 0.01 within a single σ bump). This is more positive evidence for on-chain features than the 4.5 stability profile suggested in isolation, but it is fold-specific (split 27 only). Across the 28 folds, no on-chain feature reaches 50% selection frequency (Section 4.5); but on the most recent fold, the on-chain signal (`exchange_supply_pct`) does survive into the symbolic decision function. The stronger conclusion (on-chain features carry signal that survives KAN distillation in deployment-relevant regimes) requires per-fold extraction, which is the L3 limitation in 5.4.
...
- **The joint DSR / PBO finding.** No model achieves DSR ≥ 0.95 (top is LSTM at 0.4175), but PBO sits in the robust-selection regime at 0.2571, and the leave-one-out PBO analysis shows that removing any one of the four nonlinear models (RF, XGBoost, LSTM, KAN) collapses PBO to 0.000 while removing either linear baseline leaves it unchanged. The combination is informative: rankings are stable enough to claim that the four nonlinear models beat the two linear baselines, but selection within the nonlinear group is itself unreliable across sub-path partitions, and the model that any selection rule picks does not achieve a Sharpe that survives multiple-testing correction. The honest reading is "we know the four nonlinear models cluster above the two linear baselines, we cannot reliably rank within the nonlinear group, and none of the nonlinear models is statistically significant once selection bias is corrected for". This is a stronger position than either DSR or PBO alone could deliver: we are not in a regime where rankings are random (PBO ≈ 0.5) or anti-predictive (PBO > 0.5); we are in a regime where the linear-vs-nonlinear ordering is stable but the within-nonlinear ordering and the absolute level of performance are both too uncertain to claim predictive ability.
- **Locating the variance.** The leave-one-out PBO (Section 4.4) shows that excluding any of the four nonlinear models (Random Forest, XGBoost, LSTM, KAN) collapses PBO to 0.000, while excluding either linear baseline (AR Logistic, Logistic Regression) leaves PBO unchanged at 0.257. This localises the IS/OOS instability in the competition among the four nonlinear models rather than in any single model's path-Sharpe trajectory. The reading: the four nonlinear models are interchangeable from a PBO perspective, and what looks like instability in the headline PBO of 0.2571 is the IS-best slot shuffling among them across sub-path partitions. Two implications follow. First, the strong reading "LSTM is the dominant variance source" (which would follow from any single-model exclusion analysis) is not what the leave-one-out evidence supports: every nonlinear model is equally capable of absorbing the instability when removed. Second, the linear baselines (AR Logistic, Logistic) are too far behind in median Sharpe to be candidates for the IS-best slot in any sub-path partition, so they cannot contribute to the IS/OOS instability the headline PBO measures. The honest framing is therefore "the four nonlinear models cluster as a tight group and the linear baselines cluster below them, but selection within the nonlinear group is unreliable across sub-path partitions".
- **Consistency with EMH.** The DSR < 0.95 finding aligns with semi-strong-form EMH (Fama 1970) for BTC daily direction: under leakage-free evaluation with multiple-testing correction, no architecture in the comparison demonstrates a statistically significant predictive edge. This is the intellectually honest answer to Q1 and consistent with `chassot_audrino_2026`: properly fitted baselines are hard to beat, and the literature's ML-superiority results came from suboptimal fitting schemes that handicapped the baselines. **Cite:** `fama_1970`, `chassot_audrino_2026`.
...
- **Numerical sensitivity at the dataset mean.** `exchange_supply_pct` posts the largest economically interpretable per-σ effect: a one-σ increase in BTC-on-exchanges shifts the decision function (logit) by approximately -4.55 units, which corresponds to moving P(up) from roughly 0.50 to roughly 0.01, a saturating rather than a linear effect. The sign aligns with the conventional reading that more BTC on exchanges signals selling pressure. `chaikin_osc` registers a numerically large σ-effect (~10¹⁰) that is a scaling artefact: the locked run did not log-transform Chaikin oscillator the way it does OBV, so the formula coefficients carry the raw cumulative scale. The economically meaningful sensitivity ranking is therefore `exchange_supply_pct` (-4.55 σ-effect on logit, saturating Δp) > `silver_ret_30` (-0.072 σ-effect, σ Δp -0.018) > `eth_btc_ratio` (+0.0019 σ-effect, σ Δp +0.0005) > `chaikin_osc` (effectively un-interpretable without rescaling). Worth flagging in 5.4 as a methodological refinement: log-transform Chaikin oscillator alongside OBV in future runs.
...
- One paragraph stating the headline: under leakage-free evaluation, the literature's 85% to 95% accuracy claims do not survive; top DSR is LSTM at 0.4175, far below the 0.95 significance threshold. PBO at 0.2571 indicates that model selection is robust across IS/OOS sub-path partitions for the linear-vs-nonlinear contrast, but the leave-one-out PBO analysis shows that removing any one of the four nonlinear models (RF, XGBoost, LSTM, KAN) collapses PBO to 0.000, indicating that selection within the nonlinear group is itself unreliable. This is the honest answer to Q1: the four nonlinear models cluster as a tight group and beat the two linear baselines, but ranking within the nonlinear group is not reproducible, and no model achieves a Sharpe that survives multiple-testing correction.
- One paragraph on what does survive: the symbolic extraction pipeline produces a humanly auditable closed-form decision function on four features, with one on-chain feature (`exchange_supply_pct`) showing the largest economically interpretable effect (a one-σ bump moves the logit by -4.55, driving P(up) from approximately 0.50 to approximately 0.01, a saturating bearish signal). The closed-form approximation costs 4.5 percentage points of validation accuracy relative to the trained spline network, but produces an artefact (with symbolic derivatives, term decomposition, and audit trail) that no black-box model in the comparison can produce. Interpretability is a separable contribution from predictive accuracy.
