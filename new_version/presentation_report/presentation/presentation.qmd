              ...
              {"title": dict(text=_tbl_title(pt, nd)),
               "sliders": [dict(active=_nd_vals.index(nd),
                   ...
                   steps=[dict(method="update", label=str(nd2),
                       ...
                       for nd2 in _nd_vals],
                   pad=dict(t=40, b=10), x=0.0, len=1.0, font=dict(size=11))]}]))
...
fig2.update_layout(
    updatemenus=[dict(type="buttons", direction="right", x=0.0, y=-0.75, xanchor="left", yanchor="top",
                      buttons=buttons_pt, font=dict(size=10, color="#d0d0d0"),
                      bgcolor="rgba(30,60,110,0.7)", bordercolor="rgba(59,130,246,0.5)",
                      active=-1)],
    sliders=[dict(active=1, currentvalue=dict(prefix="num_days: ", font=dict(size=12)),
                  steps=default_slider_steps, pad=dict(t=40, b=10), x=0.0, len=1.0, font=dict(size=11))],
    template="plotly_dark", height=500, margin=dict(l=50, r=30, t=70, b=170),
...
fig3.update_layout(
    updatemenus=[dict(type="buttons", direction="right", x=0.0, y=-0.75, xanchor="left", yanchor="top",
                      buttons=buttons_cap, font=dict(size=10, color="#d0d0d0"),
                      bgcolor="rgba(30,60,110,0.7)", bordercolor="rgba(59,130,246,0.5)",
                      active=-1)],
    sliders=[dict(active=1, currentvalue=dict(prefix="time_decay: ", font=dict(size=12)),
                  steps=default_decay_steps, pad=dict(t=40, b=10), x=0.0, len=1.0, font=dict(size=11))],
    template="plotly_dark", height=470, margin=dict(l=50, r=30, t=70, b=170),
...
<h4 style="color: #eee8d5; margin: 0;">Purging (AFML §7.1)</h4>
<p style="font-size: 0.68em; color: #b0b0b0; margin: 0;">
...
<h4 style="color: #eee8d5; margin: 0.3em 0 0 0;">Embargo (AFML §7.4)</h4>
<p style="font-size: 0.68em; color: #b0b0b0; margin: 0;">
...
<img src="cpcv_embargo.png" style="width: 100%; border-radius: 6px; border: 1px solid rgba(255,255,255,0.08); margin-top: auto;" />
</div>
...
_cm_names = list(_cm_data.keys())
fig_cm = make_subplots(rows=2, cols=4,
    subplot_titles=_cm_names[:3] + ["Sharpe Ratio Distribution"] + _cm_names[3:] + [""],
...
<h4 style="color: #eee8d5; margin-top: 0.8em; font-size: 0.75em;">Decision Function &nbsp;→&nbsp; P(up) = 1/(1+exp(−x))</h4>
...
```{mermaid}
%%{init: {'theme': 'dark', 'flowchart': {'padding': 30, 'nodeSpacing': 50, 'rankSpacing': 50, 'useMaxWidth': true, 'htmlLabels': true, 'wrappingWidth': 200}, 'themeVariables': {'primaryColor': '#2a5a8a', 'primaryTextColor': '#e0e0e0', 'primaryBorderColor': '#3b82f6', 'lineColor': '#93c5fd', 'secondaryColor': '#1a4a3a', 'tertiaryColor': '#4a3a1a', 'fontFamily': 'system-ui', 'fontSize': '16px', 'mainBkg': '#2a5a8a', 'nodeBorder': '#3b82f6'}}}%%

graph TD
    subgraph P1[" Phase 1 : Pre-CPCV "]
        direction LR
        A[" Data Retrieval "] --> B[" CUSUM Filter "]
        B --> C[" Triple Barrier "]
        C --> D[" Sample Weights "]
        D --> E[" Feature Eng "]
    end
    ...
    subgraph P2[" Phase 2 : CPCV "]
        direction LR
        F[" CPCV Splits "] --> G[" FFD Scale MDA "]
        G --> H[" Model Training "]
    end

    subgraph P3[" Phase 3 : Evaluation "]
        direction LR
        I[" Path Stitching "] --> J[" Comparison "]
        J --> K[" Symbolic Extr "]
        K --> L[" P = sigmoid "]
    end
    ...
    style P1 fill:#1a3a5c,stroke:#3b82f6,stroke-width:2px,color:#e0e0e0
    style P2 fill:#1a3a2a,stroke:#10b981,stroke-width:2px,color:#e0e0e0
    style P3 fill:#3a3a1a,stroke:#f59e0b,stroke-width:2px,color:#e0e0e0
    style L fill:#4a3a1a,stroke:#f59e0b,stroke-width:2px,color:#f2a900
```
