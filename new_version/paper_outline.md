...
- **Cochrane: structure.** Lead with the contribution. Three pages maximum for the introduction. Lit review is brief, set off, and positioned against the 2 to 3 closest papers per stream. Tables and figures get self-contained captions. Two to three significant digits. Less math is better.
...
### 1.3 Problems Q1 to Q4 (T-R6 parallel logic with C1 to C4)

- **Q1 Predictability under leakage-free evaluation.** Does BTC daily direction remain predictable once labels stop leaking and overlapping observations are downweighted? Does any positive Sharpe survive DSR, PBO, and DeLong?
- **Q2 Which feature families carry signal.** Among technical, statistical, macroeconomic, and on-chain features, which survive multi-model permutation-importance selection across CPCV folds?
- **Q3 KAN vs standard model families.** Where does a KAN sit relative to AR Logistic, Logistic Regression, Random Forest, XGBoost, and LSTM under identical CPCV splits, features, sample weights, and metrics?
- **Q4 Closed-form formula extraction.** Can a human-readable expression for P(Up) be extracted from a CPCV-trained KAN while preserving most of the predictive accuracy?

### 1.4 Contributions C1 to C4

- **C1 → Q1. Honest evaluation under full AFML.** CUSUM event sampling, triple-barrier labels, sample weights (uniqueness × return attribution × time decay), fractional differencing, CPCV with purging and embargo, DSR, PBO, and DeLong corrections.
...
- One short paragraph naming Sections 2 to 7 (optional per Cochrane; keep minimal if included).
...
- **2.1.2 KAN architecture (Liu 2024).** MLPs put fixed activations on nodes; KANs put learnable activations on edges. Each edge is a B-spline `g(x) = Σ a_k · B_k(x)` of order `k` over `G` grid intervals. Architecture notation `[n_input, n_hidden, n_output]`. Training is Adam followed by LBFGS with L1-on-activations and entropy sparsity. Key property: after training, each edge's spline can be inspected, pruned, or replaced with a symbolic function. **Cite:** `liu_kan_2024`.
- **2.1.3 KAN 2.0 and MultKAN.** Introduces multiplication nodes alongside addition; enables multiplicative interactions without log-exp decomposition. Future-work hook. **Cite:** `liu_kan2_2024`.
...
- **2.1.5 Training best practices.** Coarser grids generalise better on small data; L1+entropy regularisation aids pruning; Adam-to-LBFGS staging; tanh input normalisation matches default grid range `[-1, 1]`. **Cite:** `noorizadegan_2026`.
- **2.1.6 Broader landscape.** Catch-all for TKAN, ChebyKAN, DecoKAN, and other variants. TKAN as canonical recurrent-KAN variant. The comprehensive review notes symbolic distillation in financial time series is underexplored. **Cite:** `yamak_et_al_2025`, `genet_inzirillo_2024`.
- **2.1.7 Gap statement.** No prior work applies KANs to BTC daily direction. No prior work extracts symbolic formulas from a classification KAN. No prior work combines KAN symbolic extraction with AFML evaluation.
...
- **2.2.1 Overview.** Three families (statistical/econometric, classical ML, deep learning); two framings (price-level regression, direction classification).
- **2.2.2 TA + ML for BTC.** Confluence study identifies ROC as a top predictor (justifies `roc_14` inclusion). Limitation: standard split, no purging. **Cite:** `mate_confluence_2024`.
- **2.2.3 DL for BTC direction.** Same problem as this thesis with DL models; reports competitive LSTM accuracy. Limitation: fixed-horizon labels, no CUSUM, no sample weights, no purging or embargo. **Cite:** `omole_enke_2024`.
- **2.2.4 Broader crypto-DL landscape.** Catch-all citation. Recurring issues: small datasets, overfitting, no transaction costs, no risk-adjusted metrics, inconsistent evaluation. **Cite:** `bourday_crypto_dl_2024`, `wu_crypto_dl_review_2024`.
- **2.2.5 Conservative prediction in noisy regimes.** Nabar and Shroff (2023): in noisy financial settings, selective abstention beats always-predict. Motivates the bet-sizing threshold mechanism in 3.10.1 (predictions with `p ≈ 0.50` produce `bet ≈ 0`, structural abstention). **Cite:** `nabar_shroff_2023`.
- **2.2.6 Methodological weaknesses [bridge to 2.3].** Synthesise the common problems: naive train-test splits with overlapping labels; fixed-horizon labelling; no sample weighting; accuracy as primary metric; no multiple-testing correction.

### 2.3 The AFML Methodology

> Opening claim: AFML treats financial ML as a methodology problem first and a model problem second.

- **2.3.1 Overview and motivation.** Lopez de Prado (2018) identifies systematic flaws (backtest overfitting, label-overlap leakage, naive CV, inflated Sharpes). **Cite:** `lopez_de_prado_2018`.
- **2.3.2 CUSUM event filter.** Symmetric CUSUM (Snippet 2.4) identifies structurally meaningful events; threshold calibrated as multiple of mean daily volatility.
- **2.3.3 Triple barrier labelling.** Three barriers (take-profit, stop-loss, vertical); label = first barrier touched; carries `t1` for purging. **Cite:** `lopez_de_prado_2018` Ch. 3, `kang_kim_2025` (TBL on Korean equities, cross-asset validation).
- **2.3.4 Fractional differentiation.** Integer differentiation destroys long memory; price levels preserve memory but are non-stationary. FFD finds minimum `d* ∈ (0, 1)` that achieves stationarity while preserving memory. **Cite:** `lopez_de_prado_2018` Ch. 5.
- **2.3.5 Sample weights.** Concurrent labels → uniqueness → return-attribution weights → time decay. Corrects redundancy in overlapping labels. **Cite:** `lopez_de_prado_2018` Ch. 4.
- **2.3.6 Combinatorial Purged Cross-Validation.** Standard k-fold is invalid because overlapping TBL labels leak across folds. CPCV: `N` contiguous groups, `k` test groups → `C(N,k)` splits, `φ(N,k)` backtest paths. Purging removes train observations whose labels overlap with test (three sufficient conditions). Embargo removes a buffer after each test group. Path matrix enables distribution-level Sharpe analysis. **Cite:** `lopez_de_prado_2018` Ch. 7 and 12.
- **2.3.7 DSR and PBO.** DSR corrects observed Sharpe for selection bias across `n_trials` models and for non-normal returns; DSR > 0.95 is significant after correction. PBO via combinatorially symmetric CV: split paths into IS/OOS halves, check if IS-best underperforms OOS median; PBO ≈ 0 reliable, PBO > 0.5 adversarial. **Cite:** `lopez_de_prado_2018` Ch. 11 and 14.
- **2.3.8 AFML applied to crypto.** Slepaczuk and Bieganowski (2024) combine FFD and TBL with supervised autoencoders on BTC/ETH/LTC; Fu et al. (2024) use a GA-driven TBL for pair trading. Both use walk-forward instead of CPCV and neither computes DSR or PBO. This thesis fills the evaluation gap. **Cite:** `slepaczuk_bieganowski_2024`, `fu_et_al_2024`.
- **2.3.9 The fitting scheme matters more than model choice (HARd to Beat).** Audrino and Chassot: across 1,445 US equities, a properly fitted HAR model outperforms RF, XGBoost, GBT, and FFNN for realised-volatility forecasting; ML superiority results came from suboptimal fitting schemes. Directly relevant: this thesis's negative DSR results align with the same finding. **Cite:** `chassot_audrino_2026`, `corsi_2009`.
- **2.3.10 Gap statement.** Few crypto-ML studies adopt the full AFML pipeline; no study combines AFML and KAN for any asset; no study reports DSR and PBO for KAN-based predictions. This thesis is the first to apply the complete pipeline to BTC direction prediction with KANs and five baselines.
...
>
> Target length: 18 to 20 pages of prose.

### Pipeline overview

- Single short paragraph naming the three phases. Use the table below for the at-a-glance map.
...
- **CUSUM truncation.** Raw series begins 2014-11-01 to provide the 252-day lookback the longest-warmup features require. The CUSUM event filter is then truncated to start 2015-08-08, the first day with valid ETH/USD data needed for `eth_btc_ratio`. CUSUM accumulators are computed on the full raw series; only the event-firing window is restricted.
...
> Theory in 2.3.2 and 2.3.3. This section covers locked-run configuration only.

- **3.2.1 Daily volatility (Snippet 3.1).** EWMA std of log returns, `span=50`. Two roles: target width for TBL barriers; CUSUM threshold calibration.
- **3.2.2 CUSUM event filter (Snippet 2.4).** Symmetric `s_pos`, `s_neg` on the full raw series. Threshold `h = 1.0 × mean(daily_vol)`. After firing, events truncated to start 2015-08-08. Locked-run: 1,352 candidates → 82 dropped by truncation → 1,270 enter TBL.
- **3.2.3 Triple barrier labelling (Snippets 3.2, 3.4, 3.5).** `pt_sl=(1.5, 1.5)` symmetric, `num_days=10`, `min_return=0.02`. Output `bins[ret, bin, t1]`. Observed mean holding period 5.0 days; vertical barrier hit on 19.7% of events.
- **3.2.4 Rare label removal (Snippet 3.8).** `min_pct=0.085` combined with symmetric `pt_sl` and `min_return=0.02` eliminates class 0 → binary labels {-1, +1}. **Final: 1,168 events** at `{+1: 664, -1: 504}` = 56.85% Up / 43.15% Down.
...
> Theory in 2.3.5 (AFML Chapter 4). This section lists the four steps with locked-run parameter values.
...
- **Figure.** Sample weights over time, annotated with notable BTC events (March 2020 COVID, 2021 ATH, 2022 FTX).
...
- **3.4.1 TA (25).** Returns and volatility (8): `log_returns`, realised vol, Garman-Klass, Yang-Zhang, ATR (EWMA span=14, log-transformed), Bollinger width, `vol_term_7_30`, `vol_term_30_90`. Momentum/trend (9): RSI(14), MACD/signal/hist, `roc_14`, Stoch %K/%D, Williams %R, CCI(14). Volume (3): OBV (log-transformed), Chaikin, MFI(14). Distribution shape (2): rolling skew/kurt (window=21). Trend ratios (3): EMA 20/50, EMA 50/200, VWMA 20/50. Standard TA periods kept (no per-indicator tuning to avoid overfitting). **Cite:** `garman_klass_1980`, `yang_zhang_2000`, `mate_confluence_2024`.
- **3.4.2 Mathematical (9, AFML Part 4).** Information-theoretic (3): Shannon entropy, Lempel-Ziv complexity, negentropy. Random-walk (2): Hurst, variance ratio. Normality (1): Jarque-Bera. Structural breaks (3): SADF, SMT-polynomial-1, SMT-exponential. Cached due to O(n²) cost. **Cite:** `lopez_de_prado_2018` Part 4, `lo_mackinlay_1988`.
- **3.4.3 External (29).** Macro (20): `dxy_roc_30`, `us2y`, `us10y`, yield curves, `vix`, plus 30-day and 14-day return windows on seven commodity/index series (SP500, Nasdaq, gold, silver, copper, oil, natgas). Crypto-macro (1): `eth_btc_ratio` from CoinMetrics (with yfinance fallback). On-chain (8): `active_addr_roc_14`, `tx_count_roc_14`, `hashrate_roc_30`, `mvrv`, `net_exchange_flow`, `fee_per_tx`, `exchange_supply_pct`, `issuance_ntv`. All shifted by 1 day. Anti-leakage via `merge_asof(direction='backward')`.
- **3.4.4 Lag (10).** `AR_LAGS = [1, 2, 3, 4, 5, 6, 7, 14, 21, 30]`. Lag features compete with engineered features in MDA. AR Logistic selects the 10 lag columns by name from the pre-MDA matrix.
- **3.4.5 Phase I output.** Log transforms applied to `atr` and `obv` only. NaN values from warmup and external gaps are not dropped in Phase I; per-fold resolution happens inside CPCV (3.6.1). Alignment `align_for_cv(features, bins, weights) → (X, y, w, t1)` via index intersection, with hard assertions (non-empty intersection, monotone index, identical lengths, no all-NaN columns, `t1` fully populated, weights > 0). **Output.** `X` shape `(1168, 73)`, `y` shape `(1168,)`, `w` shape `(1168,)`, `t1` shape `(1168,)`.
- **Table.** Alignment summary (daily bars → CUSUM events → labelled events → aligned size).

### 3.5 Cross-Validation Framework

> Theory in 2.3.6. This section covers the locked-run configuration, the path-matrix construction, and the leakage audit.

- **3.5.1 Why standard CV fails.** Single short paragraph: TBL labels span `[t0, t1]` multi-day intervals; overlapping spans leak training-label information into the test period; metrics inflate. Refer back to 2.3.6.
- **3.5.2 CPCV configuration (with path matrix).** `N_GROUPS=8`, `K_TEST_GROUPS=2`, `EMBARGO_PCT=0.01`. Groups 0 to 6 of size `⌊T/N⌋`; group 7 absorbs remainder. `C(8, 2) = 28` splits; `φ(8, 2) = C(7, 1) = 7` backtest paths. Each group appears in 7 test sets at 146 events per group. Justification for N=8, k=2: yields 146 events per group while keeping the training fold at approximately 858 events (73.4% of 1,168) after purging and embargo; 28 splits and 7 paths give denser combinatorial diversity for PBO than smaller N values. Path matrix construction per AFML Section 12.4.1: for group `g`, path `p` uses the `p`-th split that includes `g` in its test set; each path uses 4 of the 28 splits and the Sharpe distribution across the 7 paths feeds DSR and PBO. Locked-run audit: average train 858 events, test 292, purged 1.9, embargoed 16.5; group date ranges span 2015-08-08 to 2026-05-02. **Figure.** CPCV split visualisation for representative splits (0, 13, 27).
- **3.5.3 Purging and embargo (with leakage audit).** Purging (AFML Snippet 7.1): three sufficient overlap conditions for training observation `i` against test window `[t_test_start, t_test_end]` (observation falls in window; label resolves in window; label spans the entire window). Embargo (AFML 7.4.2): `int(EMBARGO_PCT × T) = 11` observations removed immediately after each test group; one-sided because training labels resolving before the test starts contain no future test information. Leakage audit closing paragraph: zero training observations whose `t1` falls within the assigned test group's date range, across all 28 splits.
- **Table.** Leakage audit (28 rows: split id, test groups, train count, test count, leaks, status) → Appendix.
...
- **3.7.5 LSTM.** Single-layer `nn.LSTM` → final hidden state → LayerNorm → dropout → linear → 2 logits. `num_layers=1` hardcoded. Sliding window `LSTM_WINDOW=14` (close to TBL `num_days=10`); reduces sequence count from `N` to `N-13`. Tanh input normalisation matching the spline grid range. Training stack: AdamW (`weight_decay=1e-4`), CrossEntropyLoss with class + AFML weights, label smoothing 0.1, grad clip max norm 1.0, cosine annealing warm restarts (`T_0=25`, `T_mult=2`), batch 64, max 100 epochs, early stopping patience 15. Tuned: `hidden ∈ {16, 32}`, `dropout ∈ [0.1, 0.5]`, `lr` log-uniform [1e-4, 5e-2]. Tuning at 50 epochs / patience 7; production refit at 100 / 15 (compute-vs-fidelity trade-off). 5 seeds, 40 trials per split. **Cite:** `hochreiter_schmidhuber_1997`, `loshchilov_adamw_2019`.
...
- Nested per-split Optuna inside each CPCV training fold (AFML Ch. 7 compliant: outer CPCV provides unbiased test folds; inner tuning operates entirely within the training fold).
...
- Per-split tuned params overwrite module-level constants before training; pristine defaults restored on subsequent runs to prevent contamination.
...
- **Unweighted calibration locked.** Audit showed weighted Platt/vector pushed every model's calibration miss further from the base rate and compressed Sharpes; weighted code paths preserved but `sample_weight=None` in the calibrator call site.
...
>
> Theory for DSR and PBO in 2.3.7; this section covers the configuration and the methodological choices.

#### 3.10.1 Trading simulation

- **Bet sizing (AFML 10.3, compressed to 2 sentences + formula).** `sign = +1 if P(up) > P(down) else -1`; confidence `p = max(P(0), P(1))`; z-score `z = (p - 0.5) / sqrt(p(1-p) + 1e-10)`; raw bet `2·Φ(z) - 1`; clip to [-0.75, 0.75]; threshold `|bet| < 0.05 → 0`; discretise to {0, 0.25, 0.5, 0.75}; apply sign. Implicit abstention per Conservative Predictions (Section 2.2.5). **Cite:** `lopez_de_prado_2018` Ch. 10, `nabar_shroff_2023`.
- **Strategy returns.** `gross = bet · ret_label`; `turnover = |Δbet|`; `tx_cost = 0.001 · turnover`; `net = gross - tx_cost`. 0.1% per trade is conservative for major BTC exchanges (maker 0.02 to 0.10%, taker 0.04 to 0.10%). Annualise at 365 (BTC trades 24/7); risk-free rate 0.
- **Path stitching (AFML 12.4.1).** 7 backtest paths assembled from the 28 splits via `path_map[path_id]`. For each `(group_id, split_id)`: retrieve predictions AND filter to events whose positional index falls within `group_bounds[group_id]`. The group filter prevents duplication when co-tested groups appear in multiple splits. Multi-seed: average calibrated probabilities across 5 seeds before bet sizing (variance reduction ≈ 1/√5). Concatenate, sort, assert no duplicate timestamps.[^stitching] **Cite:** `lopez_de_prado_2018` Ch. 12.
...
| Metric | Formula |
|--------|---------|
| Annualised Sharpe | `(mean / std) · √365` |
| Cumulative return | `Π(1 + r_t) - 1` |
| Annualised return | `(1 + cum_ret)^(1/years) - 1` |
| Max drawdown | `min((equity - running_max) / running_max)` |
| Time under water | longest run below previous peak (days) |
| Win rate | fraction of traded observations with positive return |
| Profit factor | `Σ(positive) / |Σ(negative)|` |
...
#### 3.10.3 Statistical correction layer

- **DSR (AFML Ch. 14).** Refer to 2.3.7 for the formula. `n = n_trials = 6` (compared models); `n_obs = avg_n_returns` per path. Kurtosis convention: Mertens (2002) raw kurtosis; conversion applied from `scipy.stats.kurtosis` excess. NaN-safety clamp on variance term. DSR > 0.95 → significant after correction.
- **PBO via CSCV (AFML Ch. 11).** Refer to 2.3.7. Path-Sharpe matrix shape (6 models × 7 paths). All `C(7, 3) = 35` IS/OOS partitions (IS=3 paths, OOS=4 paths). PBO = fraction where IS-best underperforms OOS median. N=8 yields 35 partitions vs 10 under N=6, raising resolution from 0.10 to 0.029 per partition.
...
#### 3.11.1 Purpose, fold selection, data preparation
...
#### 3.11.2 Four-step PyKAN algorithm

> Following the four-step algorithm of `cho_lee_kim_2025` (see also `noorizadegan_2026`): train → prune → symbolify → affine fine-tune. Architecture matches CPCV-benchmarked KAN.

- **Step 1. Three-phase training.** Phase 1: Adam (`lr=1e-3`, `wd=1e-3`), 600 steps, Gaussian noise (std 0.05) clamped to [-1, 1], early stopping. Phase 2a: LBFGS (`lr=0.01`), 20 steps, no regularisation. Phase 2b: LBFGS, 20 steps, L1 + entropy (`λ=0.002`), encourages sparsity for Step 2. Grid extension disabled (540 samples insufficient). Dynamic majority-baseline gate after Phase 1 and pre-prune: `val_acc` checked against `max(0.53, val_majority_baseline + 0.01)`, logs warning if missed.
...
- SymPy simplification with 30-second timeout (prevents hang on pathological expressions). `nsimplify(tolerance=1e-3)` for rational coefficients.
...
#### 3.11.4 Fragilities and limitations
...
- **Acknowledged limitations (also 5.3).** Fold-specific (different folds may produce different formulas). Small sample (540 events) limits identifiable complexity; motivates the R²=0.30 threshold. Post-symbolic accuracy may fall below pre-symbolic when the symbolic library fits the trained spline less faithfully than the spline fitted the data.
...
> Lead with the main result. Numbers are from the locked end-to-end run (v6, May 2026). Drop all cross-version commentary; report locked-run figures only.
...
### 4.2 Path Sharpe distribution

- Per-model row × 7 paths + median + std + bootstrap CI. KAN: median 0.5588, std 0.7539, CI crosses zero. AR Logistic: 0.3806, 0.7355, CI crosses zero narrowly. LSTM: 0.1541, 0.6388. XGBoost: 0.0493, 0.5830 (tightest trained CI, crosses zero by small margin). Random Forest: 0.0287, 0.7533. Logistic Regression: -0.2173, 1.5969 (widest CI). Buy-and-hold: 2.2722, 0.5768, CI strictly positive.
- Sortino reorders: AR Logistic 0.3894, KAN 0.3211, LSTM 0.1189, XGBoost 0.0259, RF 0.0257, LR -0.1872. Calmar compresses trained models into -0.04 to +0.09 band; KAN leads at 0.0820.

### 4.3 Classification metrics

- Mean F1 ranges 0.4070 (KAN) to 0.4666 (AR Logistic). Pooled AUC ranges 0.4942 (XGBoost) to 0.5149 (AR Logistic across two pair contexts).
- DeLong pairwise table (15 pairs). 2 significant at α=0.05 (both involve XGBoost as lower-AUC half). Effect-size disclosure: AUC range across all six models is 0.021, comparable to seed-to-seed and split-to-split variation within any single model.
- Multiple-testing correction: Bonferroni threshold α/15 ≈ 0.0033; neither significant pair clears. Benjamini-Hochberg with FDR=0.05 also rejects.
- Confusion matrices aggregated → Appendix D.

### 4.4 Financial performance

- Equity curves: 6 models + buy-and-hold overlay on median path; log-scale y-axis. XGBoost, RF, Logistic Regression end below 1.0 (median cum returns -0.0027, -0.0579, -0.0797). AR Logistic 0.0852 (best trained); KAN 0.0841 (second).
- Median max-drawdown: -0.2172 (XGBoost) to -0.3874 (LR). KAN at -0.2466 (second-best among trained).
- Additional metrics table: cum return, ann return, max DD, time under water, win rate, profit factor, mean |bet|, % traded.
- DSR computation detail for top-ranked trained model (KAN) → text + Appendix D.

### 4.5 Feature selection and FFD stability

- **Feature stability profile.** Of 73 candidate features: 0 stable (> 80% folds), 2 moderate (50 to 80%): `eth_btc_ratio` at 57.1%, `log_returns_lag6` at 53.6%. 71 in low bucket. 0 never selected.
...
- **Group breakdown of moderate features.** 1 crypto-macro (`eth_btc_ratio`), 1 lag (`log_returns_lag6`). 0 TA, 0 macro, 0 on-chain, 0 mathematical.
- **Q2 on-chain finding.** No on-chain feature reaches 50% selection. The free-lunch hypothesis does not survive multi-model MDA + CPCV.
- **FFD stability.** ATR is the only FFD-treated column. Across 28 folds × 5 seeds = 140 d* estimates: mean ≈ 0.198, std ≈ 0.085, range [0.050, 0.400], modal 0.15 to 0.20. `std(d*) < 0.1` consistent. No fold required d*=1.0.
- **Figure.** Horizontal bar chart of 73 features by selection frequency, coloured by group.
- **Methodological reading.** Feature-selection turnover high; FFD-d* turnover low. The noise-vs-signal ratio for individual features changes across regimes, but ATR's underlying persistence is stable. Validates FFD-only-on-ATR (3.6.1) and multi-model MDA over single-model SFI (3.6.3).

### 4.6 Symbolic extraction results (Q4 answer)

- **Fold used.** Split 27 (last fold, deployment-proxy). KAN F1 on this fold: 0.3232.
- **Surviving 3 features.** `eth_btc_ratio` (57%), `log_returns_lag6` (54%), `natgas_ret_30` (39%). 70 features excluded by stability ranking.
- **Training sample.** 540 train + 135 val. Architecture `[3, 3, 2]` (15 active edges, ~90 spline parameters, parameter-to-sample ratio 6.0×, above the 5× floor).
- **Training diagnostics.** Phase 1 val_acc 0.4815 (gate fails, threshold 0.5693). Phase 2a val_acc 0.4296 (drops below Phase 1). Phase 2b val_acc 0.4667 (pre-prune gate fails, threshold 0.5359). Grid extension skipped. Full phase log → Appendix F.
- **Pruning.** No edges pruned (15 of 15 active). Pruned architecture `[[3,0],[3,0],[2,0]]`. Post-prune val_acc 0.4667.
- **Symbolification rate.** 100% (15 of 15 edges clear R² ≥ 0.30). R² distribution: min 0.734, median 0.990, max 1.000. Sin primitive dominates the top of the ranking.
- **Pre vs post-symbolic accuracy.** Pre 0.4667 (below 50% baseline). Post 0.5185 (Δ = +5.18 pp, but still below 56.85% majority baseline). The affine fine-tuning step is doing genuine work but the underlying model on this fold did not extract directional signal.
- **The decision function.** Numbered equation (full closed form). Six primitives (`sin`, `cos`, `tanh`, `x²`, `x³`, `x⁴`) across four outer terms with distinct outer wrappers (squared, sin, cos, tanh).
- **Numerical sensitivity at the dataset median (table).** Per-σ effect on logit and linearised P(Up) shift for each surviving feature. `log_returns_lag6` largest per-σ shift (-0.0983); `eth_btc_ratio` second (-0.0598); `natgas_ret_30` smallest (+0.0179). All three features have finite, well-defined gradients at the dataset median (the earlier `abs`-primitive non-differentiability is resolved).
- **Q2 evidence (on-chain).** 0 on-chain features survive extraction. Hardens Section 4.5 finding.
- **Q2 evidence (lag).** A lag feature (`log_returns_lag6`) reaches the surviving set, validating the lag-features-in-MDA-pool design choice.
- **Section closer.** "100% symbolification rate on three features; post-symbolic accuracy 51.85% (+5.18 pp over pre-symbolic 46.67%) but still below the 56.85% majority baseline. The closed-form formula faithfully captures a model that did not extract directional signal on the deployment fold. RQ4 conclusion: closed-form interpretability is achievable (clean primitives, finite gradients, three economically interpretable features), but interpretability does not imply predictive power."

---

## 5. Discussion

> Interpret results. Link findings back to the methodological mechanisms in Section 3.

### 5.1 Interpreting Q1 and Q3 results

- **Joint DSR / PBO reading.** Top trained DSR 0.2470 (KAN), far below 0.95. PBO 0.657 places model selection in the adversarial regime: the IS-best is the OOS-loser in roughly two of three partitions. Bootstrap evidence: 6 of 7 CIs cross zero; only buy-and-hold strictly positive. No trained model has a statistically significant predictive edge.
- **LOO PBO asymmetry.** Random Forest is PBO-neutral; its median Sharpe is second-lowest and its path-Sharpe distribution is wide enough that it rarely wins on any three-path IS subset. Logistic Regression is the largest destabiliser (Δ -0.286): widest bootstrap CI and occasional IS-best wins that the OOS median does not confirm.
- **DeLong significance.** 2 of 15 pairs reach α=0.05 (both involve XGBoost as the lower-AUC half). XGBoost's pooled AUC 0.4942 is the only trained-model AUC below chance. Neither pair survives Bonferroni; the per-pair information is preserved but the family-wise claim is not.
...
- **Conservative Predictions framing.** The bet-sizing threshold operationalises selective abstention (`nabar_shroff_2023`): `p ≈ 0.50 → bet ≈ 0`. KAN's mean P(Up) 0.5421 against base rate 0.5685 (within tolerance). Only one model flagged by calibration audit (XGBoost at Δ = -0.0371). The bet-sizing curve operates on calibrated probabilities; the negative DSR is a statement about signal strength, not probability quality.

### 5.2 KAN in context (Q3, Q4)

- **KAN ranks 1st among trained models** on median Sharpe (0.5588) and on DSR (0.2470). Second-best median max drawdown (-0.2466). Highest median win rate (0.5315, statistical tie with AR Logistic at 0.5318).
- **No bootstrap-strict-positive trained model.** All six trained-model CIs cross zero; only buy-and-hold's stays strictly positive. KAN's CI (-0.4677, 1.1607) widens on the lower side.
- **Where KAN's rank-1 trained Sharpe comes from.** Classification metrics mid-pack (mean acc 0.5318, mean AUC 0.5103). Rank-1 Sharpe comes from bet-sizing translation: well-calibrated mean P(Up) (0.5421 vs 0.5685 base rate), top-three win rate, and conservative max-drawdown.
- **Contrast with `oad_kasper_2025`.** KASPER reports Sharpe 12.02 on individual stocks with regime detection. This thesis reports KAN median Sharpe 0.56 on BTC direction without a regime layer and under full AFML statistical correction. The 21× gap reflects asset class, target type, and evaluation rigour rather than KAN architectural capacity.
- **Contrast with `cho_lee_kim_2025`.** VIX KAN extracts formulas that reveal mean-reversion and leverage effects, validated by domain knowledge. This thesis extracts a three-feature mixed-primitive structure on BTC daily direction where domain knowledge does not directly validate any specific functional form. The pipeline transfers across asset classes; interpretive yield depends on target structure.
...
- **The +5.18 pp gain in magnitude vs level.** Largest absolute gain across locked configurations but operates from a sub-baseline pre-symbolic level. The affine fine-tuning step is doing genuine work; the underlying model on this fold has not found signal. The symbolic representation faithfully captures a model that has not.
...
- **L6 abs-primitive non-differentiability.** Documented in 3.11.4; resolved in current run (no abs in surviving primitives) but the fragility remains in the pipeline.
- **L7 Calibration cross-section.** Five of six models within calibration tolerance; XGBoost flagged. Per-fold calibration diagnostic remains the next-step audit.

---

## 6. Conclusion

> Do not restate all findings. End with resonance. Target ≤ 10% of textual part (~3 pages).
...
- **Paragraph 3: what survives.** The symbolic-extraction pipeline produces a closed-form decision function on three features (`eth_btc_ratio`, `log_returns_lag6`, `natgas_ret_30`) with 100% symbolification rate. Post-symbolic accuracy 51.85% (+5.18 pp over pre-symbolic 46.67%) but still below 56.85% majority baseline. Six primitives (`sin`, `cos`, `tanh`, `x²`, `x³`, `x⁴`) across four outer terms; finite gradients everywhere. Lag-feature finding reproduces across locked configurations, validating the lag-features-in-MDA-pool design choice. No on-chain feature survives extraction: the Q2 free-lunch hypothesis fails at both stability and deployment-fold levels. Interpretability is a separable contribution from predictive accuracy.
- **Closing sentence with resonance.** Template: "X is not a constraint, but a catalyst, for Y." E.g., "Methodological honesty is not a constraint on financial machine learning; it is the prerequisite for treating interpretability as a contribution in its own right."
...
- **C. Hyperparameter search spaces** (model summary table; per-model justifications for tree depth, leaf size, LSTM single-layer, KAN width cap, etc.).
...
- **F. Symbolic extraction detailed output** (PyKAN training logs, edge-by-edge R², pruned architecture diagrams, unsimplified `logit_bearish` and `logit_bullish` expressions, additional PyKAN fragilities, output-dictionary documentation).
...
- [ ] No footnotes for parenthetical comments (path-stitching footnote is the only methodological exception).
...
- [ ] All cross-references resolve (Section 2.3.6 from 3.5, Section 2.3.7 from 3.10.3, etc.).
