...
**Compression transforms.** Two transforms are applied in fixed order: `np.arcsinh` (smooth signed compression) to `obv` and `chaikin_osc`, and `np.log(|x| + 1e-8)` to `atr`. The two volume-flow features (OBV, Chaikin) span 10⁷-10¹⁰ over the 2014-2026 period as BTC daily volume grew by six orders of magnitude; without compression, linear models fit on raw values produce vanishing coefficients on otherwise informative features because the gradient is dominated by the largest observations. asinh is preferred over the previously-used symmetric-log `sign(x) · log(|x|+1)` on three theoretical grounds: it is smooth at zero (no kink), it is parameter-free (no additive constant to choose), and it preserves more of the small-`|x|` structure because `asinh(x) ≈ x` near zero. For large `|x|` it is asymptotically equivalent to `sign(x) · log(2|x|)`, so the tail compression rate is identical to the previous transform up to a constant offset of `log(2) ≈ 0.69`. Reference: Burbidge, Magee & Robb (1988). ATR keeps `log(|x| + 1e-8)` because asinh and log are nearly identical on its 0.001-0.1 range; the small additive constant prevents `log(0)` for the rare zero-volatility bar. All other features are on bounded or dimensionless scales that the per-fold RobustScaler handles adequately.
