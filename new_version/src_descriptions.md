...
- `build_feature_matrix(df)`: orchestrates `compute_ta_features` + `compute_math_features` + `pd.concat`. Returns 34 columns (25 TA + 9 math), with NO compression transforms applied; the notebook applies `apply_sym_log` and `apply_log` itself with the column lists visible at the call site. External features (22) and lag features (10) are assembled separately in the notebook and concatenated alongside before alignment; the canonical concat order is `[ta, math, external, lag]`. Earlier iterations of this module included `apply_log_transforms` (and briefly `apply_asinh`) which both applied the compression transforms inside `build_feature_matrix`; both were removed in favour of the explicit notebook-level transform application.
...
The final per-feature MDA is the average across both classifiers and all inner folds. Features with positive MDA are kept (permuting them hurts at least one model on average), capped at the top 30% by MDA value (the locked configuration), with a hard floor of 5 features minimum. The cap was tightened from the common 0.40-0.50 default in an earlier methodology round on the 66-feature pool, after a high-PBO run revealed only ~6 features clearing 50% selection frequency in the stability bar chart, indicating that the long tail was contributing variance rather than signal. At that pool size, 0.25 vs 0.30 was evaluated as a sensitivity pair: both leave predictive metrics (mean accuracy, pooled AUC) essentially unchanged across all six models, but path-Sharpe-derived metrics (Sharpe, DSR, PBO) diverge substantially. 0.30 was selected as primary because it produced lower PBO (0.26 vs 0.69), indicating more robust model selection under the AFML interpretation. The May 2026 expansion of the feature pool from 66 to 73 columns (the 14d/30d macro pairs and the AR_LAGS extension) was an advisor-driven structural change; the cap was held at 0.30 across this transition, unchanged from the pre-expansion primary configuration. A 0.20 tightening was evaluated as a sensitivity check on the expanded pool (motivated by an absolute-count argument: 0.20 × 73 ≈ 15 features per fold restores the count to roughly the pre-expansion working point of 0.30 × 66 ≈ 20). The 0.20 sensitivity result shifts KAN's median Sharpe from +0.38 to +0.84 and pulls LSTM down from +1.34 to +0.60; PBO is essentially unchanged (0.34 vs 0.37). The locked configuration is `MDA_TOP_K_FRAC = 0.30` on grounds of methodological parsimony (no cap change from the pre-expansion primary); the 0.20 result is reported as a sensitivity check showing that the qualitative findings (BH dominance, no DSR > 0.95, KAN ranks above AR Logistic) are stable across cap settings while individual model point estimates and one model's CI-zero-crossing behavior reshuffle.
...
**Buy-and-hold benchmark.** `compute_buy_and_hold_summary(predictions, path_map, n_paths, reference_model)` produces a model-summary-compatible row using the same CPCV path structure the models see. For each path it reconstructs the chronological sequence of test-fold returns from the predictions dict (timestamps and per-event returns are model-invariant for a given split/seed, so any model serves as a reference), applies bet_size = 1.0 for every event (always 100% long), incurs transaction cost only on the initial buy of each path, and computes path-level metrics identically to the model paths. Returns NaN for the predictive metrics (F1, accuracy, AUC, log loss, Brier) since the benchmark makes no probabilistic predictions, and NaN for DSR since the metric requires a Sharpe under multiple-trials selection. The benchmark is added to the comparison table from the notebook by appending the resulting dict to `analysis["all_summaries"]` and re-running `compare_models(...)`. Its purpose is to contextualise the model Sharpe ratios against the trivial strategy of holding BTC throughout each test path. Note: the benchmark is a more aggressive position-size baseline than the models, which cap at `MAX_BET_SIZE = 0.75` via the S-curve, so beating buy-and-hold's 1.0-leveraged Sharpe is methodologically harder than the asymmetry would suggest.
...
- `print_symbolic_decision(symbolic)`: prints the decision function, P(up) formula, and surviving features. The headline outputs likely to be quoted directly in the thesis text.
- `print_extraction_metrics(symbolic)`: prints pre/post symbolic accuracy, symbolification rate, and the pruned architecture. The accuracy gap (pre minus post) measures the cost of replacing spline activations with closed-form symbolic primitives.
...
- `compute_feature_sensitivity(symbolic, X, eval_point="mean")`: returns a per-feature sensitivity DataFrame with `mean_value`, `std_value`, `d_decision/d_feature_at_mean`, `sigma_effect_on_decision`, and `approx_sigma_delta_p` columns. Set `eval_point="median"` to evaluate gradients at the per-feature median, which is more robust for skewed distributions and typically avoids the singularity (see "singular-gradient handling" below).
...
- `plot_marginal_effects(symbolic, X, n_points=100, quantile_low=0.05, quantile_high=0.95)`: returns a matplotlib figure with one subplot per surviving feature, sweeping each feature across its empirical [q_low, q_high] range while holding the others at their dataset medians. Returns the figure rather than calling `plt.show()` so the caller can save it for the thesis appendix.
...
**Singular-gradient handling (`_safe_eval_at_point` and `compute_feature_sensitivity`).** PyKAN's symbolic library includes `1/x`, `log(x)`, and similar reciprocal/logarithmic primitives that produce poles in the learned activation. When the symbolic gradient is evaluated at a point near such a pole — which is common for heavily right-skewed features whose mean lands far from the bulk of the distribution, e.g. `jarque_bera` with mean ≈ 218 and std ≈ 775 — `float(deriv.subs(point))` returns `inf` or `-inf`, which then propagates silently through the `sigma_effect` and `approx_delta_p` columns, producing `-inf` rows that look like a numerical bug to a thesis reader. The `_safe_eval_at_point` helper catches `ZeroDivisionError`, `TypeError`, `ValueError`, and `OverflowError` from sympy substitution and applies an `np.isfinite` check, returning NaN for any non-finite result. The display helpers render NaN as `"   N/A "` and emit a footer line counting the singular features. The `eval_point="median"` kwarg on `compute_feature_sensitivity` provides a robust alternative for heavily-skewed distributions: the per-feature median typically falls in the bulk of the distribution and avoids the singularity. An earlier version of the sensitivity computation lived inline in the notebook with no NaN-safety, which produced `-inf` in three columns of the table for any feature whose distribution had a near-pole at the mean; the helpers here are the canonical replacement.
