...
\newacronym{isoos}{IS/OOS}{In-Sample / Out-of-Sample}
...
This thesis applies a Kolmogorov-Arnold Network (KAN) to Bitcoin (BTC) price direction prediction within the full Advances in Financial Machine Learning (AFML) framework of \citet{lopez_de_prado_2018}. On the data side, the framework uses Cumulative Sum (CUSUM) event sampling, triple-barrier labeling (TBL), fractional differencing (FFD), and sample weighting. On the evaluation side, it uses Combinatorial Purged Cross-Validation (CPCV), the Deflated Sharpe Ratio (DSR), the Probability of Backtest Overfitting (PBO), and pairwise DeLong Area Under the Curve (AUC) tests. The KAN is benchmarked against five baselines: Autoregressive (AR) Logistic, Logistic Regression (LR), Random Forest (RF), Extreme Gradient Boosting (XGBoost), and Long Short-Term Memory (LSTM). All six models run under identical CPCV splits with $N=8$ groups, $k=2$ test groups per split, 28 splits, and 7 backtest paths. The locked configuration uses 5 seeds across all six models, producing 840 prediction entries. The novel deliverable is a closed-form symbolic decision function for the probability of an upward move, $P(\textup{Up})$, extracted from the trained KAN.

Under leakage-free evaluation with multiple-testing correction, no model achieves $\textup{DSR} \geq 0.95$. The top DSR among trained models is 0.2470 (KAN). Buy-and-hold posts a higher median Sharpe (2.2722) than every trained model, though it is excluded from the DSR computation because it does not enter the selection pool. PBO sits in the adversarial regime at 0.657. The in-sample best model underperforms the out-of-sample median in 23 of the 35 IS/OOS partitions, so model selection across this six-model cross-section is actively misleading in nearly two of three sub-path partitions. Of the 7 bootstrap 95\% confidence intervals on the median Sharpe, 6 cross zero. Only buy-and-hold's interval, $(1.8903, 2.4685)$, stays strictly positive. The KAN interval is $(-0.4677, 1.1607)$. Leave-one-out PBO identifies Random Forest as the anchor model ($\Delta = 0.000$). Every other model's exclusion reduces PBO, with Logistic Regression the largest destabiliser ($\Delta = -0.286$, returning the cross-section to the moderate-overfitting band). Two of the 15 pairwise DeLong AUC tests reach $\alpha = 0.05$: AR Logistic versus XGBoost ($p = 0.0427$) and Random Forest versus XGBoost ($p = 0.0318$), both with XGBoost on the lower-AUC side.

The symbolic extraction pipeline succeeds. All 15 edges symbolify cleanly, giving a 100\% symbolification rate. Three features survive: \texttt{eth\_btc\_ratio}, \texttt{log\_returns\_lag6}, and \texttt{natgas\_ret\_30}. Post-symbolic validation accuracy is 51.85\%, a 5.18 percentage-point gain over the pre-symbolic 46.67\%. However, both numbers sit well below the 56.85\% majority baseline and close to the 50\% baseline. The closed-form expression combines six primitives, $\sin$, $\cos$, $\tanh$, $x^2$, $x^3$, and $x^4$, all everywhere differentiable on the standardised feature support. The PyKAN model itself fails its Phase-1 dynamic accuracy gate ($\max(0.53,\,\textup{base\_rate} + 0.01) = 0.5693$) at validation accuracy 0.4815, and the pre-prune gate also fails. The closed-form formula therefore faithfully captures a model that did not extract directional signal on the deployment fold.
...
This thesis asks whether BTC price direction remains predictable once labels and features stop leaking the future and overlapping observations are weighted properly. It evaluates the claim against three tests on a six-model cross-section: the DSR, the PBO, and pairwise DeLong AUC comparisons.
...
This thesis asks where the KAN sits in a six-model cross-section evaluated under identical CPCV splits, features, sample weights, and metrics. The five baselines span four families: econometric (AR Logistic), linear ML (LR), tree-based (RF, XGBoost), and recurrent deep learning (LSTM).
...
This thesis asks whether a human-readable mathematical expression for the probability of an upward BTC price move, $P(\textup{Up})$, can be extracted from a CPCV-trained KAN while preserving most of its predictive accuracy. It evaluates this by running PyKAN's four-step train-prune-symbolify-tune pipeline on the last CPCV fold and comparing pre- and post-symbolic validation accuracy.
...
\textbf{C1.} This thesis applies the complete AFML stack to BTC direction prediction. Data preparation uses CUSUM event sampling, triple-barrier labeling, fractional differencing, and sample weights that combine uniqueness, return attribution, and time decay. Evaluation uses CPCV with purging and embargo, the DSR, the PBO, and pairwise DeLong AUC tests. Every leakage and multiple-testing correction is implemented in a single reproducible workflow.

% C2

\textbf{C2.} This thesis assembles a 73-feature universe spanning four families: 25 technical, 9 statistical (AFML Part 4), 29 external (20 macroeconomic, 1 crypto-macro, and 8 on-chain from CoinMetrics), and 10 autoregressive lag features. All 73 features compete in multi-model permutation importance selection across the 28 CPCV folds. No family is privileged a priori.

% C3

\textbf{C3.} This thesis benchmarks a KAN against five baselines: AR Logistic, LR, RF, XGBoost, and LSTM. All six models run under identical CPCV splits, identical features, identical sample weights, and identical metrics. Pairwise DeLong AUC tests determine which differences across the six-model cross-section are statistically significant.

% C4

\textbf{C4.} This thesis extracts a human-readable mathematical expression for $P(\textup{Up})$ from the trained KAN, using PyKAN's prune-and-symbolify pipeline. To the author's knowledge, this is the first closed-form classification formula derived from a CPCV-trained KAN in a weak-signal regime. It is the primary novel contribution of this work.


\subsection{Thesis Outline}\label{subsec:structure}
