...
Calibration is fitted on the held-out 10% calibration partition of the training fold (chronological split, no shuffling, separate from the 10% validation partition used for early stopping). Never touches test data. If calibration fails for any reason, the pipeline falls back to uncalibrated `predict_proba`.

### Methodological note

Under the locked 80/10/10 three-way split, the 10% calibration partition serves Platt/vector scaling exclusively and is fully separate from the 10% validation partition that handles XGBoost's early stopping and the LSTM/KAN best-state tracking. An earlier 80/20 split used a single 20% subset for both purposes, on the rationale that early stopping only controls ensemble size and Platt/vector scaling fits at most two-three parameters, so the shared use introduced minimal leakage. The three-way split removes the dual-use coupling at the cost of fewer events per role (~75 per partition rather than ~150) and is the locked configuration.
...
**Three-way 80/10/10 chronological split for early stopping and calibration.** Within each training fold, the data is split chronologically into 80% model-train, 10% validation, and 10% calibration. The validation partition feeds the model's `X_val` / `y_val` for early stopping and best-state tracking; the calibration partition is held entirely separate and feeds Platt or vector scaling. An earlier 80/20 split used the same 20% subset for both early stopping and calibration, which introduced subtle leakage between the model's stopping decision and the calibration data it was later fitted against. The three-way split removes this coupling at the cost of slightly fewer events per role (~75 events per partition rather than ~150).
...
The ~2,200-line file contains extensive defensive code for PyKAN's inconsistent APIs:
