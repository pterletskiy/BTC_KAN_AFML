...
\begin{center}
\textbf{BITCOIN DIRECTION PREDICTION USING KOLMOGOROV-ARNOLD NETWORKS WITHIN THE AFML FRAMEWORK BY PETR TERLETSKIY} \\
...
\textbf{C1.} This thesis applies the complete AFML stack to BTC direction prediction: CUSUM event sampling, triple-barrier labeling, sample weights (uniqueness × return attribution × time decay), fractional differencing, Combinatorial Purged Cross-Validation with purging and embargo, and the Deflated Sharpe Ratio, Probability of Backtest Overfitting, and DeLong AUC tests. The pipeline implements every leakage and multiple-testing correction in a single reproducible workflow.
