...
This thesis applies a Kolmogorov-Arnold Network (KAN) to Bitcoin (BTC) price direction prediction within the full Advances in Financial Machine Learning (AFML) framework of \citet{lopez_de_prado_2018}, which comprises Cumulative Sum (CUSUM) event sampling, triple-barrier labeling (TBL), fractional differencing (FFD), sample weighting, Combinatorial Purged Cross-Validation (CPCV), the Deflated Sharpe Ratio (DSR), the Probability of Backtest Overfitting (PBO), and pairwise DeLong Area Under the Curve (AUC) tests. The KAN is benchmarked against five baselines, Autoregressive (AR) Logistic, Logistic Regression (LR), Random Forest (RF), Extreme Gradient Boosting (XGBoost), and Long Short-Term Memory (LSTM), under identical CPCV splits with $N=8$ groups, $k=2$ test groups per split, 28 splits, and 7 backtest paths; the locked configuration runs 5 seeds across all six models, producing 840 prediction entries. The novel deliverable is a closed-form symbolic decision function for the probability of an upward move, $P(\textup{Up})$, extracted from the trained KAN.

Under leakage-free evaluation with multiple-testing correction, no model achieves $\textup{DSR} \geq 0.95$. The top DSR among trained models is 0.2470 (KAN), and buy-and-hold posts a higher median Sharpe (2.2722) than every trained model, though it is excluded from the DSR computation because it does not enter the selection pool. PBO sits in the adversarial regime at 0.657: the in-sample best model underperforms the out-of-sample median in 23 of the 35 IS/OOS partitions, so model selection across this six-model cross-section is actively misleading in nearly two of three sub-path partitions. Of the 7 bootstrap 95\% confidence intervals on the median Sharpe, 6 cross zero. Only buy-and-hold's interval, $(1.8903, 2.4685)$, stays strictly positive; the KAN interval is $(-0.4677, 1.1607)$. Leave-one-out PBO identifies Random Forest as the PBO-anchor ($\Delta = 0.000$): every other model's exclusion reduces PBO, with Logistic Regression the largest destabiliser ($\Delta = -0.286$, returning the cross-section to the moderate-overfitting band). Two of the 15 pairwise DeLong AUC tests reach $\alpha = 0.05$: AR Logistic versus XGBoost ($p = 0.0427$) and Random Forest versus XGBoost ($p = 0.0318$), both with XGBoost on the lower-AUC side.

The symbolic extraction pipeline succeeds with 100\% symbolification rate (15 of 15 edges, no skips) on three surviving features: \texttt{eth\_btc\_ratio}, \texttt{log\_returns\_lag6}, and \texttt{natgas\_ret\_30}. Post-symbolic validation accuracy is 51.85\%, a 5.18 percentage-point gain over the pre-symbolic 46.67\%, but both numbers sit well below the 56.85\% majority baseline. The closed-form expression combines six primitives, $\sin$, $\cos$, $\tanh$, $x^2$, $x^3$, and $x^4$, and is everywhere differentiable on the standardised feature support. The PyKAN model nonetheless fails its Phase-1 dynamic accuracy gate ($\max(0.53,\,\textup{base\_rate} + 0.01) = 0.5693$) at validation accuracy 0.4815, and the pre-prune gate fails as well, so the symbolic representation faithfully captures a model that did not extract directional signal on the deployment fold.
...
\textbf{Q1.} Many recent BTC ML studies report daily-direction accuracies of 85 to 95 percent \citep{omole_enke_2024}. These results come from traditional ML pipelines, which rely on fixed-horizon labels, overlapping label spans, and train-test splits that do not purge concurrent observations or apply an embargo across fold boundaries. Under these conditions, training labels can carry information about test-period prices and the high reported accuracies may be inflated by an unknown margin.

This thesis asks whether BTC price direction remains predictable once labels and features stop leaking the future and overlapping observations are weighted properly. It evaluates the claim against three tests on a six-model cross-section: the Deflated Sharpe Ratio (DSR), the Probability of Backtest Overfitting (PBO), and pairwise DeLong Area Under the Curve (AUC) comparisons.

% Q2

\textbf{Q2.} Most cryptocurrency ML studies rely on one or two feature families, typically technical indicators or price histories fed into neural networks and other ML models \citep{bourday_crypto_dl_2024}. There is no consensus on whether macroeconomic, crypto-macro, or on-chain features add information beyond price-derived inputs. Without a uniform evaluation across families, the question of which families actually carry signal remains unanswered.

This thesis asks which feature families carry signal under a leakage-free evaluation, and whether on-chain features deliver the "free lunch" over price-derived inputs. It answers both by recording per-family selection frequencies from multi-model permutation importance across the 28 Combinatorial Purged Cross-Validation (CPCV) folds, covering technical, mathematical, macroeconomic, crypto-macro, on-chain, and autoregressive lag inputs.

% Q3

\textbf{Q3.} KANs have been applied to VIX forecasting \citep{cho_lee_kim_2025}, stock prediction \citep{oad_kasper_2025}, and cryptocurrency regression \citep{gao_decokan_2025}, but never to BTC direction classification. KANs' position relative to econometric, linear, tree-based, and deep-learning baselines is unknown. Whether the predictive strengths reported in other financial time-series tasks carry over to BTC direction under a uniform protocol is therefore untested.

This thesis asks where the KAN sits in a six-model cross-section evaluated under identical CPCV splits, features, sample weights, and metrics. The five baselines span four families: econometric (AR Logistic), linear ML (Logistic Regression), tree-based (Random Forest, Extreme Gradient Boosting), and recurrent deep learning (Long Short-Term Memory).

% Q4

\textbf{Q4.} Existing KAN symbolic-extraction work in finance targets regression problems on relatively predictable series, such as VIX forecasting \citep{cho_lee_kim_2025}, stock prediction \citep{oad_kasper_2025}, and cryptocurrency price modelling \citep{gao_decokan_2025}. No published work, to the author's knowledge, has extracted a closed-form classification formula from a CPCV-trained KAN in a weak-signal regime. Whether this pipeline carries over from regression on predictable series to classification under leakage-free evaluation is therefore untested.

This thesis asks whether a human-readable mathematical expression for the probability of an upward BTC price move (P(Up)) can be extracted from a CPCV-trained KAN while preserving most of its predictive accuracy. It evaluates this by running PyKAN's four-step train-prune-symbolify-tune pipeline on the last CPCV fold and comparing pre- and post-symbolic validation accuracy.
...
\textbf{C1.} This thesis applies the complete AFML stack to BTC direction prediction: CUSUM event sampling, triple-barrier labeling, sample weights combining uniqueness, return attribution, and time decay, fractional differencing, Combinatorial Purged Cross-Validation with purging and embargo, and the Deflated Sharpe Ratio, Probability of Backtest Overfitting, and DeLong AUC tests. The pipeline implements every leakage and multiple-testing correction in a single reproducible workflow.

% C2

\textbf{C2.} This thesis assembles a 73-feature universe spanning four families: 25 technical, 9 statistical (AFML Part 4), 29 external (20 macroeconomic, 1 crypto-macro, 8 on-chain from CoinMetrics), and 10 autoregressive lag features. All 73 features compete in multi-model permutation importance selection across the 28 CPCV folds, with no family privileged a priori.

% C3

\textbf{C3.} This thesis benchmarks a KAN against five baselines (AR Logistic, Logistic Regression, Random Forest, Extreme Gradient Boosting, Long Short-Term Memory) under identical CPCV splits, identical features, identical sample weights, and identical metrics. Pairwise DeLong AUC tests determine which differences across the six-model cross-section are statistically significant.

% C4

\textbf{C4.} This thesis extracts a human-readable mathematical expression for P(Up) from the trained KAN via PyKAN's prune-and-symbolify pipeline. To the author's knowledge, this is the first closed-form classification formula derived from a CPCV-trained KAN in a weak-signal regime, and the primary novel contribution of this work.


\subsection{Document Structure}\label{subsec:structure}

Section 2 reviews the literature on KANs, ML for BTC prediction, and the AFML methodology. Section 3 details the data and the full empirical pipeline. Section 4 reports the results, Section 5 discusses them, Section 6 concludes, and Section 7 outlines directions for future work.
...
The methodology unfolds in three sequential phases. Phase I, the pre-CPCV stage, transforms the raw 4,208 daily OHLCV bars into a smaller set of aligned observations. It covers data acquisition (Section 3.1), event filtering and triple-barrier labelling (Section 3.2), AFML sample weighting (Section 3.3), feature engineering (Section 3.4), and the alignment step that produces the final tuple of feature matrix $X$, labels $y$, weights $w$, and barrier-touch timestamps $t_1$. 

Phase II runs the CPCV pipeline. It assembles the per-split training and test folds (Section 3.5), applies per-fold preprocessing (Section 3.6), trains the six benchmark models with nested hyperparameter tuning (Sections 3.7 and 3.8), calibrates the predicted probabilities (Section 3.9), and produces a test-fold prediction stream for each model across all 28 splits (Section 3.10). 

Phase III applies the post-CPCV evaluation and interpretability layer. It assembles the seven backtest paths, computes the deflated Sharpe ratio and the probability of backtest overfitting, runs pairwise DeLong AUC tests (Section 3.11), and extracts the closed-form symbolic decision function from the trained KAN (Section 3.12). 

The full pipeline is shown as a diagram in \phantomsection\label{ref:pipeline_call}\autoref{fig:pipeline_diagram}.


\subsection{Data}\label{subsec:data}

The base instrument is Bitcoin priced in US dollars (BTC-USD). Daily Open-High-Low-Close-Volume (OHLCV) bars are retrieved from Yahoo Finance, spanning 2014-11-01 to 2026-05-09 (4,208 daily bars). The start date is chosen to satisfy the longest-warmup feature in the engineered set, the 50/200 exponential moving average ratio at 200 days. Every shorter-warmup feature inherits a clean starting point as a consequence.
...
The download is validated before entering the labelling pipeline. Empty responses, duplicate dates, and OHLCV inconsistencies abort the pipeline with an explicit error rather than producing silent failures. Calendar gaps of three days or fewer are forward-filled to preserve the daily sampling rate. Longer gaps abort the pipeline as well. Any row with a missing close is dropped.

All rolling windows operate on the BTC trading calendar of 7-day weeks, 30-day months, 90-day quarters, 180-day semesters, and 365-day years. Unlike equity markets, BTC trades 24/7, so the calendar contains no weekend or holiday gaps. The 365-day convention flows downstream into the Sharpe-ratio annualisation in Section 3.11.2 and into the DSR reference distribution.

Three external data sources extend the feature set beyond price and volume. They are described in detail in Section 3.4 and summarised here only by source. Macroeconomic series (yield curves, the CBOE Volatility Index (VIX), commodity prices, the dollar index) come from Yahoo Finance and the Federal Reserve Economic Data (FRED) database. The crypto-macro ETH/BTC ratio comes from the CoinMetrics Community API with Yahoo Finance as a fallback. The eight on-chain features (active addresses, transaction count, hash rate, MVRV, net exchange flow, fee per transaction, exchange supply fraction, and native-unit issuance) come from the CoinMetrics Community API.

Two anti-leakage measures are applied at this stage. All external series are aligned onto BTC's calendar by a backward-looking join. For each BTC trading day, the alignment uses only the most-recent external observation timestamped at or before that day, and never a future value. CoinMetrics observations are then shifted by one trading day to respect the end-of-day reporting convention of the CoinMetrics Community API. A metric timestamped at $t$ is only published at the close of $t$, and therefore enters the feature matrix at $t+1$.

\subsection{Labelling}\label{subsec:labeling}

The labelling pipeline transforms the 4,208 daily bars into a smaller set of binary-labelled observations through three sequential constructs that recur throughout the rest of the methodology. An \textit{event} is a CUSUM-triggered timestamp at which the cumulative drift in log returns crosses a volatility-scaled threshold (Section 3.2.2). A \textit{label} is the triple-barrier output assigned to each event, encoding which of three barriers was touched first (Section 3.2.3). An \textit{observation} is the final aligned sample at an event timestamp, combining the label, the engineered features, the AFML sample weight, and the barrier-touch timestamp $t_1$ used for downstream purging.


\subsubsection{Daily volatility}\label{subsubsec:vol}

The daily volatility series is computed first because two later steps depend on it. It is estimated as the exponentially weighted moving average (EWMA) standard deviation of log returns, with span 50 \citep[Snippet 3.1]{lopez_de_prado_2018}. The half-life of an EWMA with span 50 is approximately 17 days, giving the estimator a memory of roughly two trading weeks \phantomsection\label{ref:ewma_call}(\autoref{fig:ewma_vol}).

The span is shortened from the AFML default of 100 days for equities to 50 days for BTC. Two empirical features of the asset motivate the change. BTC undergoes faster regime transitions than equities, driven by the four-year halving cycle and by regulatory shocks that compress volatility responses into days rather than months. A span of 100 days would smooth those transitions out and underestimate volatility during regime shifts. A span of 50 days tracks them more responsively, at the cost of marginally higher estimator variance.

Two downstream uses depend on this series. First, the CUSUM threshold in Section 3.2.2 is calibrated as a multiple of the mean daily volatility, so that the event filter fires at a roughly constant signal-to-noise ratio across the sample. Second, the triple-barrier widths in Section 3.2.3 are scaled by the daily volatility at each event, so that upper and lower barriers expand and contract with the prevailing regime rather than sitting at a fixed return percentage. The volatility series therefore appears twice in the labelling pipeline, once as a calibration constant and once as a width parameter.

\subsubsection{CUSUM event filter}\label{subsubsec:cusum}

The CUSUM filter selects the timestamps at which the cumulative drift in log returns crosses a volatility-scaled threshold. The bars at these timestamps are retained for triple-barrier labelling. Everything between events is discarded. Two accumulators are maintained in parallel, one for upward drift and one for downward drift. The upward accumulator $S^+_t$ adds positive returns at each new bar and floors at zero. The downward accumulator $S^-_t$ adds negative returns and ceils at zero. When either crosses its threshold $h$, an event fires at that timestamp and the offending accumulator resets to zero. The implementation follows AFML Snippet 2.4 \citep{lopez_de_prado_2018}.

The threshold is set to $h = 1.0 \times \overline{\sigma}$, where $\overline{\sigma}$ is the mean of the daily volatility series from Section 3.2.1 over the labelling window. The choice was tightened from the AFML default of $1.5 \times \overline{\sigma}$ after an empirical sweep over the range $\{0.5, 1.0, 1.5, 2.0, 3.0\}$. A multiplier of $0.5$ produces too many events and reduces the average information content per event. A multiplier of $3.0$ produces too few, leaving long stretches of the sample without any event. A multiplier of $1.0$ yields a workable event count with approximately balanced classes after triple-barrier labelling. The mechanism is illustrated over a three-month window in early 2026 \phantomsection\label{ref:cusum_call}(\autoref{fig:cusum_window}).

The accumulators are computed on the full raw return series from 2014-11-01 onwards, but the event-firing window is truncated to start on 2015-08-08. The truncation reflects two constraints. The 50/200 exponential moving average ratio needs 200 calendar days of price history to populate (Section 3.1), so the earliest a fully-populated feature vector can exist is May 2015. The ETH/USD price series, which feeds the ETH/BTC ratio feature, only begins on 2015-08-08 with the Ethereum Frontier launch. The truncation date is the later of the two constraints and therefore satisfies both. The accumulators themselves run on the full pre-2015-08-08 history, so the events that survive truncation reflect the genuine state of the cumulative drift at the time they fire, not a reset on 2015-08-08.

The locked-run figures follow from these choices. On the full series, the CUSUM filter fires 1,352 candidate events. Truncation drops the 82 candidates that fire before 2015-08-08, leaving 1,270 events that enter the triple-barrier labelling step in Section 3.2.3. Rare-label removal drops a further 102 events whose labels collapse to class zero under the minimum-return threshold, giving the final 1,168 binary-labelled observations. The pipeline therefore compresses 4,208 daily bars to 1,168 observations, a ratio of roughly 28\%.


\subsubsection{Triple barrier labelling}\label{subsubsec:tbl}

The triple-barrier method assigns a label to each CUSUM event by simulating a path-dependent trading position opened at the event timestamp and closed at the first of three barriers it touches. The upper horizontal barrier represents the profit-taking level. The lower horizontal barrier represents the stop-loss level. The vertical barrier represents the maximum holding period. The label is $+1$ if the upper barrier is hit first, $-1$ if the lower barrier is hit first, and the sign of the realised return at expiry if the vertical barrier is reached. The implementation follows AFML Snippet 3.2 \citep{lopez_de_prado_2018}.

The horizontal barriers are placed at $\pm 1.5 \times \sigma_t$ around the entry price, where $\sigma_t$ is the daily volatility series from Section 3.2.1 evaluated at the event timestamp. The symmetric profit-take and stop-loss multipliers reflect the absence of a directional bias in the labelling step. The prediction problem itself remains directional and is delegated to the models in Section 3.7. The vertical barrier is set 10 calendar days after the event, which is approximately two trading weeks on the BTC calendar. Vertical-barrier expiries whose absolute return falls below a minimum threshold of 0.02 are assigned a transitional label of zero, which is removed in the rare-label step described below.

The output of the triple-barrier method is a table indexed by event timestamp, with three columns. The realised return $ret$ records the price change between entry and exit. The label $bin \in \{-1, 0, +1\}$ records which barrier was touched first, modulated by the minimum-return threshold for vertical-barrier hits. The barrier-touch timestamp $t_1$ records the calendar day on which the position closed and is used for the purging and embargo steps of the cross-validation framework in Section 3.5.1. Three representative events, one for each barrier type, are shown in \autoref{fig:tbl_examples}.
...
The empirical distribution of barrier hits validates the parameter choices. The mean holding period is 5.0 days against a maximum of 10, with a median of 4 days. Only 19.7\% of events expire at the vertical barrier, which means the horizontal barriers close approximately 80\% of positions before the time-out. The 5-day mean holding period also keeps the average barrier-touch timestamp clear of the next CUSUM event, which limits the magnitude of the AFML uniqueness correction applied in Section 3.3.

The residual class-zero events are then dropped in a rare-class removal step. The step drops any class whose fraction in the post-labelling dataset falls below a minimum threshold. The threshold is set to 0.085, raised from the AFML default of 0.05 to deliberately drop class zero. The 102 vertical-barrier ties between the 0.02 minimum return and zero represent 8.0\% of the post-truncation event count, which falls just below the 0.085 threshold and so triggers the drop. The pipeline therefore delivers a strict binary classification problem $bin \in \{-1, +1\}$, with no neutral-return events to manage downstream.

The locked-run figures complete the chain from Section 3.2.2. The 1,270 events that survive the CUSUM truncation enter triple-barrier labelling. The minimum-return threshold collapses 102 of them into class zero, which the rare-label step then drops, leaving 1,168 final binary-labelled observations. The class balance settles at 56.85\% Up against 43.15\% Down (\phantomsection\label{ref:label_dist_call}\autoref{fig:label_distribution}), which is close enough to balanced for stratified loss functions to behave well without additional class weighting.


\subsection{Sample Weights}\label{subsec:weights}

The triple-barrier labelling of the previous section produces overlapping observations. The 10-day vertical barrier means a position opened at event $i$ remains open through the days that may host subsequent CUSUM events, so two consecutive events sharing part of their barrier windows are partially redundant. Training on overlapping observations without correction overweights densely-clustered events and biases the gradient estimate towards the most active periods of the market. AFML proposes a four-step sample-weighting scheme that addresses both this redundancy and the asymmetric information content across events \citep[Chapter 4]{lopez_de_prado_2018}. A fifth step, outlier capping, is added to control the tail of the weight distribution.

The first step counts the number of overlapping label spans at each calendar day. For each bar $t$, the concurrent label count $c_t$ records how many event positions remain open on day $t$ (Snippet 4.1). 

The second step turns this count into a per-event uniqueness score. The average uniqueness $\bar{u}_i$ of event $i$ is the mean of $1/c_t$ over the days inside its barrier window (Snippet 4.2). An event whose span is shared with no others has $\bar{u}_i = 1$ and contributes the full weight of its information. An event whose span is fully shared with two others has $\bar{u}_i \approx 1/3$ and is correspondingly downweighted in the gradient.

The third step scales uniqueness by absolute return. The return-attribution weight $w_i = \bar{u}_i \times |ret_i|$ rewards events with large realised moves, since these carry more directional information than near-zero returns (Snippet 4.10). The weights are then normalised so that their sum equals the number of events, which keeps the standard fitting interface stable across different sample sizes. 

The fourth step applies a linear time decay. Old events are downweighted by a factor that decreases linearly from one at the most recent event to 0.4 at the oldest, with the slope set so that the average decay is unity (Snippet 4.11). The time-decay factor of 0.4 is chosen to honour the regime non-stationarity of BTC without discarding the historical record entirely. The asset has matured substantially over the labelling window, evolving from a primarily retail-driven market in the mid-2010s into one with structural institutional participation by the early 2020s. Recent observations therefore carry more information about current price dynamics than equally-aged observations from earlier periods. The 0.4 floor still allows pre-institutional behaviour to inform the models without giving it equal weight. The two multiplicative scalings combine into a per-event weight $w_i = \bar{u}_i \times |ret_i| \times d_i$, where $d_i$ is the decay factor at event $i$.

The fifth step caps extreme weights at the 99th percentile of the post-decay distribution. The cap is necessary because the multiplicative chain can produce a heavy upper tail. Events that combine a large absolute return, high uniqueness, and a recent timestamp accumulate weights several times the mean, and a handful of these can dominate the empirical risk minimisation of an entire fold. The 99th percentile of the empirical distribution sits at approximately 2.862 in the locked run, and 12 events hit the cap. 

Notable instances include the COVID-19 crash on 2020-03-08 (return $-38.7\%$), the FTX collapse on 2022-11-04 (return $-12.3\%$), and the early-2019 rally on 2019-03-29 (return $+19.1\%$). The weight distribution and its evolution over time are shown in \phantomsection\label{ref:weights_call}\autoref{fig:sample_weights}, and the full list of capped events appears in \autoref{tab:capped_events}.

The final per-event weight vector enters each model's training procedure through the standard weight-aware fitting interface. The ML classifiers (LR, RF, XGBoost) accept the vector through the sample-weight argument of their fit methods. The DL models (LSTM, KAN) apply the weights as per-sample multipliers inside the cross-entropy loss, so that the gradient at observation $i$ is scaled by $w_i$ before averaging across the batch. Both routes produce an empirical risk that respects the AFML-corrected information content of each observation.


\subsection{Feature Engineering}\label{subsec:features}

The feature engineering layer turns the 4,208 OHLCV bars and the external data sources of Section 3.1 into 73 numerical features per event timestamp. The features fall into four families. Twenty-five technical indicators encode the standard chartist and quantitative signals from the BTC OHLCV series (Section 3.4.1). Nine mathematical features apply AFML Part 4 transformations of returns and log prices (Section 3.4.2). Twenty-nine external features encode macroeconomic conditions, the cryptoasset cross-section, and on-chain activity, drawn from twenty macroeconomic series, one cross-asset ratio, and eight on-chain measures (Section 3.4.3). Ten lagged log-return features encode the autoregressive structure of BTC's own returns (Section 3.4.4). All 73 features are eligible candidates for the multi-model Mean Decrease Accuracy (MDA) selection procedure of Section 3.6, with the single exception that the Autoregressive Logistic baseline restricts its input to the 10 lag columns regardless of MDA results. The full per-feature catalogue, including formulas and lookback windows, sits in \autoref{app:features}.


\subsubsection{Technical analysis features (25)}\label{subsubsec:ta}

The 25 technical indicators fall into five functional categories. The largest is the returns-and-volatility group of eight features, anchored on daily log returns and including two range-based volatility estimators that improve on close-to-close estimates by using intraday extremes \citep{garman_klass_1980, yang_zhang_2000}, an EWMA-smoothed Average True Range, Bollinger Band width, and two volatility-term-structure ratios comparing 7-day to 30-day and 30-day to 90-day realised volatilities. The second-largest is the momentum-and-trend group of nine features, anchored on Wilder's Relative Strength Index and the MACD triplet, and including the 14-day rate of change identified as the top BTC predictor in \citet{mate_confluence_2024}, the Stochastic oscillator pair, Williams \%R, and the Commodity Channel Index. The remaining eight features split across three smaller groups. Three volume-based indicators cover On-Balance Volume (sign-preserving log-transformed), the Chaikin oscillator, and the Money Flow Index. Two distribution-shape features capture 21-day rolling skewness and kurtosis. Three trend ratios capture the relative position of short and long moving averages, namely the 20/50 and 50/200 exponential moving average ratios and the 20/50 volume-weighted moving average ratio.

Indicator periods are kept at their canonical values (RSI = 14, MACD = 12/26/9, CCI = 14, MFI = 14) rather than optimised over the labelling window. The rolling skewness and rolling kurtosis use a 21-day window matching one calendar month on the BTC calendar. Optimising any of these periods within the same dataset that trains the models would introduce another layer of overfitting risk, and the canonical values reflect decades of use across multiple asset classes. The 25 indicators are summarised in \autoref{tab:ta_features}.

\begin{table}[H]
\centering
\begin{tabular}{lcp{8cm}}
\toprule
Category & Count & Representative indicators \\
\midrule
Returns and volatility & 8 & log returns, GK vol, YZ vol, ATR, BB width, vol term structure \\
Momentum and trend & 9 & RSI, MACD triplet, ROC, Stoch \%K/\%D, Williams \%R, CCI \\
Volume & 3 & OBV, Chaikin oscillator, MFI \\
Distribution shape & 2 & rolling skewness, rolling kurtosis \\
Trend ratios & 3 & EMA 20/50, EMA 50/200, VWMA 20/50 \\
\bottomrule
\end{tabular}
\caption{Technical indicators grouped by functional category. Full per-indicator formulas and lookback windows are catalogued in \autoref{app:features}.}
\label{tab:ta_features}
\end{table}


\subsubsection{Mathematical features (9, AFML Part 4)}\label{subsubsec:math}

The nine mathematical features come from AFML Part 4 \citep{lopez_de_prado_2018} and capture distributional and structural properties of BTC's returns and log prices. Three features measure information content. Shannon entropy on a 30-day rolling window captures disorder, Lempel-Ziv complexity on a 90-day window captures compressibility, and negentropy on a 30-day window measures the gap between the maximum-entropy Gaussian benchmark and the empirical entropy of returns, so that positive values flag departures from Gaussianity. Low entropy and low Lempel-Ziv complexity together signal structured return behaviour, where prediction models find more learnable signal than under random noise.

Two features test the random-walk null. The Hurst exponent uses rescaled-range analysis on 180-day windows with sub-windows of 14, 30, 60, and 90 days, where values above 0.5 indicate persistence and values below indicate mean reversion. The variance ratio of \citet{lo_mackinlay_1988} uses 90-day windows with a 7-day lag, with departures from unity rejecting the random-walk null. Persistent regimes are typically easier to forecast because directional momentum carries information across consecutive bars, and rejection of the null implies multi-period returns carry exploitable autocorrelation.

The remaining four features cover normality and structural-break diagnostics. The Jarque-Bera statistic on 90-day windows tests skewness and excess kurtosis against the Gaussian null, flagging fat-tailed or skewed return episodes that violate the assumptions of common parametric models. The Supremum Augmented Dickey-Fuller (SADF) statistic identifies the most explosive sub-period within each rolling window, and the Sub- and Super-Martingale Tests (SMT) fit polynomial-1 and exponential trends to test the cumulative log-return series against a martingale null. SADF flags bubble and crash dynamics, while SMT rejection signals deterministic drift the random-walk null misses. \autoref{tab:math_features} summarises the family.

\begin{table}[H]
\centering
\begin{tabular}{lcp{8cm}}
\toprule
Category & Count & Features (rolling window in days) \\
\midrule
Information-theoretic & 3 & Shannon entropy (30), Lempel-Ziv complexity (90), negentropy (30) \\
Random-walk tests & 2 & Hurst exponent (180), variance ratio (90, lag 7) \\
Normality & 1 & Jarque-Bera (90) \\
Structural breaks & 3 & SADF (min 90, lag 1), SMT polynomial-1, SMT exponential \\
\bottomrule
\end{tabular}
\caption{Mathematical features grouped by functional category. Full per-feature formulas and rationale are in \autoref{app:features}.}
\label{tab:math_features}
\end{table}


\subsubsection{External features (29)}\label{subsubsec:external}

The 29 external features extend the BTC-only input space into the broader financial and on-chain context. Twenty macroeconomic features come from Yahoo Finance and the Federal Reserve Economic Data (FRED) database. Six features cover the rates and volatility backdrop, namely the 30-day rate of change of the dollar index, the 2-year and 10-year Treasury yields, the 2-year-to-10-year and 10-year-to-30-year yield-curve slopes, and the CBOE Volatility Index (VIX). The remaining fourteen features cover seven commodity and equity-index series, namely the S\&P 500, Nasdaq, gold, silver, copper, oil, and natural gas, each at a 30-day and a 14-day return window. The 14-day variants were added so that the multi-model MDA selection step of Section 3.6 has access to a faster macro horizon on each asset, since the previous 30-day-only configuration would otherwise leave no shorter horizon in the candidate pool.

One crypto cross-asset feature, the ETH-to-BTC price ratio, captures the largest cryptoasset's relative pricing. The ratio is computed by aligning daily ETH-USD prices onto BTC's trading calendar with a backward-looking join and dividing by BTC-USD. ETH prices come from the CoinMetrics Community API as the primary feed, with Yahoo Finance as a fallback. The CoinMetrics-primary configuration extends ETH coverage back to 2015-08-08, the Ethereum Frontier launch date, and limits the residual missing-value rate to 7.7\%, all of which sits in the November 2014 to August 2015 pre-ETH-trading window that the CUSUM start-date filter of Section 3.2.2 truncates out. An earlier BTC-dominance feature was removed when the available data source was found to return BTC market capitalisation rather than the bounded dominance percentage, and the proxy fallback could not be cleanly defended on methodological grounds.

Eight on-chain features capture the operational state of the Bitcoin network from the CoinMetrics Community API. Three features encode network activity through the 14-day rate of change of daily active addresses, the 14-day rate of change of transaction count, and the 30-day rate of change of network hash rate. Five further features encode balance-sheet and supply dynamics through the Market-Value-to-Realised-Value (MVRV) ratio, the net exchange flow, the average fee per transaction, the fraction of supply held on exchanges, and the daily native-unit issuance. All external series enter the feature matrix through the backward-looking join described in Section 3.1, and the CoinMetrics observations are additionally shifted forward by one trading day to respect the Community API's end-of-day reporting convention. \autoref{tab:external_features} summarises the family.

\begin{table}[H]
\centering
\begin{tabular}{lcp{8cm}}
\toprule
Sub-group & Count & Features \\
\midrule
Macro: rates and volatility & 6 & DXY 30-day ROC, US 2Y yield, US 10Y yield, 2Y-10Y slope, 10Y-30Y slope, VIX \\
Macro: commodities and indices & 14 & S\&P 500, Nasdaq, gold, silver, copper, oil, natural gas (30-day and 14-day returns each) \\
Crypto-macro & 1 & ETH/BTC ratio \\
On-chain: network activity & 3 & active addresses ROC 14, tx count ROC 14, hash rate ROC 30 \\
On-chain: balance sheet and supply & 5 & MVRV, net exchange flow, fee per tx, exchange supply \%, issuance \\
\bottomrule
\end{tabular}
\caption{External features grouped by source and functional category. Macro features are drawn from Yahoo Finance and FRED, and crypto-macro and on-chain features from CoinMetrics. Full per-feature definitions are in \autoref{app:features}.}
\label{tab:external_features}
\end{table}


\subsubsection{Lag features (10)}\label{subsubsec:lags}

Ten lagged log-return features encode the autoregressive structure of BTC's own returns. The lag set is $\{1, 2, 3, 4, 5, 6, 7, 14, 21, 30\}$ days, which covers a contiguous near-term spectrum out to one week and adds the two-week, three-week, and four-week milestones. The contiguous one-to-seven coverage avoids the discontinuity between lag 3 and lag 7 that an earlier sparser configuration carried, and the three-week mark at lag 21 closes the gap between the two- and four-week scales. Lag features participate in the multi-model MDA selection step of Section 3.6 on equal footing with the other 63 engineered features. This removes the information asymmetry that would otherwise arise between the Autoregressive Logistic baseline, which has natural access to lag features by construction, and the other models, which would otherwise not see lag information in their candidate pool. The Autoregressive Logistic baseline itself still selects all ten lag columns from the pre-MDA matrix by name, regardless of which features survive the MDA cut for the other models.


\subsubsection{Alignment and NaN handling}\label{subsubsec:alignment}

Two features receive a log transform before alignment. The Average True Range is replaced by its natural logarithm to compress its heavy upper tail without losing the absolute level. The On-Balance Volume is replaced by a sign-preserving log transform that brings its unbounded positive and negative cumulative excursions onto a comparable scale. All other features in the four families are either bounded or dimensionless and require no transformation.

The feature engineering layer produces rolling-window outputs with NaN values from two sources. Rolling-window warmup creates NaN values at the start of the sample where the longest-lookback features lack enough history to compute a valid value. External-data calendar gaps create NaN values where a macro or on-chain series has not yet been published at a BTC trading day. Phase I does not drop or impute these values. The alignment step that closes Phase I hard-asserts that no feature column is entirely NaN and that the barrier-touch timestamp $t_1$ is fully populated, while partial-NaN columns trigger a soft warning and pass through. Per-fold NaN imputation is deferred to inside the CPCV loop and is described in Section 3.6.1, so that the imputation policy can use only training-fold information without leaking test-fold means or medians.

The alignment step itself intersects the indices of the feature matrix, the label vector, the sample weights, and the barrier-touch timestamps to produce the four-tuple $(X, y, w, t_1)$. Several hard assertions guard the integrity of the output. The intersected index must be non-empty, free of duplicate dates, and monotonically increasing in time. The four arrays must share lengths and indices. The barrier-touch timestamps must be fully populated, the weights must be strictly positive, and the labels must lie in $\{-1, 0, +1\}$. The locked-run output is a $1{,}168 \times 73$ feature matrix paired with a 1,168-dimensional binary label vector $y \in \{-1, +1\}$, a 1,168-dimensional positive weight vector $w$, and a 1,168-dimensional barrier-touch timestamp vector $t_1$. The full sample-size chain from raw OHLCV to the aligned event dataset is given in \autoref{tab:alignment_summary}.

Phase I therefore closes with the aligned $(X, y, w, t_1)$ dataset, a chronologically-ordered set of 1,168 CUSUM-selected, binary-labelled observations ready for the cross-validation pipeline of Section 3.5.
...
\bottomrule
\end{tabular}
\caption{Sample-size chain from raw OHLCV to the aligned event dataset. CUSUM filtering drops 2,938 daily bars, rare-class removal drops 102 vertical-barrier ties below the 0.02 minimum-return threshold, and alignment introduces no further row drops in the locked run.}
\label{tab:alignment_summary}
\end{table}

\subsection{Cross-Validation Framework}\label{subsec:cpcv}

The standard k-fold cross-validation procedure is invalid for financial time series with overlapping labels. The triple-barrier labels of Section 3.2.3 span multi-day intervals, so each label depends on the price path over the entire barrier window rather than on a single timestamp. If two events sit in different folds but their barrier windows overlap on the calendar, the training label and the test label both draw on the same price moves, and information leaks across the train-test boundary. Combinatorial Purged Cross-Validation (CPCV) of \citet[Chapter 12]{lopez_de_prado_2018} prevents this leakage by purging training events whose barrier windows reach into the test fold, embargoing the calendar days immediately after the test fold, and combining multiple test-group choices into a backtest that consumes each event exactly once. The remainder of this section sets out the leakage mechanism (Section 3.5.1), the CPCV split construction adopted in this thesis, the purging and embargo rules, and the path-stitching procedure that aggregates the 28 splits into 7 non-overlapping backtest paths.


\subsubsection{Why standard CV fails}\label{subsubsec:cv_fail}

The labels produced by triple-barrier labelling are not point estimates but multi-day intervals. The label of event $i$ is determined by the price path between the event timestamp $t_{0,i}$ and the barrier-touch timestamp $t_{1,i}$, which in the locked-run distribution averages 5 days and reaches 10 days at the time-out barrier (Section 3.2.3). Every label therefore depends on the next several days of prices, not just on the day of the event.

Standard k-fold cross-validation assigns observations to folds without regard to this structure. Events are shuffled and divided into $k$ equal partitions, after which each partition takes its turn as the test fold while the others train the model. Because the shuffling ignores time, the training fold typically contains events whose barrier windows extend into the test fold. The training label of such an event reflects price moves that the test set also draws its labels from, and the model is therefore exposed to test-period prices through the training data.

The consequence is a metric that overstates true forecasting ability. A model that scores well on these test folds is not necessarily forecasting the future. It may simply be exploiting the partial agreement between its training labels and its test labels, since both draw on overlapping price paths. Reported AUC, accuracy, and Sharpe ratio on standard k-fold splits therefore inflate the genuine out-of-sample performance of any model trained with overlapping-label data.


\subsubsection{CPCV configuration}\label{subsubsec:cpcv_config}

The CPCV pipeline partitions the 1,168 aligned observations into $N = 8$ contiguous groups of 146 observations each, with $k = 2$ groups serving as the test set in each split and an embargo of 1\% applied to the calendar boundary. The number of splits is $\binom{8}{2} = 28$, and each group appears in $\binom{7}{1} = 7$ test sets, which equals the number of non-overlapping backtest paths that the 28 splits combine into (Section 3.5.4).

The choice of $N = 8$ and $k = 2$ balances three competing demands. The 146-event group size is large enough that within-fold AUC and Sharpe estimates are not dominated by sampling noise. The 28 splits provide the combinatorial diversity required by the Probability of Backtest Overfitting (PBO) test of Section 3.11.4. The 7 backtest paths give the Deflated Sharpe Ratio (DSR) of Section 3.11.3 enough independent draws to populate a stable test-statistic distribution. Reducing either $N$ or $k$ would shrink one of these three dimensions without a corresponding gain elsewhere.

Across all 28 splits the average training fold contains 858 observations (73.4\% of the labelling window) and the test fold contains 292. The purging step drops an average of 1.9 events per split, and the embargo absorbs an average of 16.5. The eight groups span 2015-08-08 to 2026-05-02, and a zero-leakage check returns clean across all 28 splits. The group partitions are shown on the BTC price chart in \autoref{fig:cpcv_groups}.


\subsubsection{Purging, embargo, and leakage verification}\label{subsubsec:purging_embargo_leakage}

Three mechanisms close the leakage channels that overlapping triple-barrier labels create across CPCV folds. Purging removes training observations whose label windows overlap the test fold. The embargo absorbs the residual serial correlation that crosses the calendar boundary after the test fold. An empirical leakage audit then verifies that the two mechanisms together leave no overlapping observations in any of the 28 splits.

Purging removes from the training fold any observation whose barrier window overlaps the test fold's calendar range \citep[Snippet 7.1]{lopez_de_prado_2018}. Three sufficient conditions cover the overlap geometry. The first condition fires when the entry timestamp $t_{0,i}$ of training observation $i$ falls inside the test window $[t_{\text{start}}, t_{\text{end}}]$. The second condition fires when the barrier-touch timestamp $t_{1,i}$ falls inside the test window. The third condition fires when the training observation's barrier window fully encloses the test window, that is, when $t_{0,i} \leq t_{\text{start}}$ and $t_{\text{end}} \leq t_{1,i}$. Any training observation satisfying at least one condition is dropped from the training fold for that split.

The embargo removes a fixed number of training observations immediately after each test group. With an embargo of 1\% and 1,168 total observations, this drops 11 training observations per test-group boundary, namely those whose entry timestamps fall inside the embargo window. The embargo is applied only after the test group, not before, because training labels that resolve before the test starts cannot carry information about future test prices. The mechanism guards against residual serial-correlation leakage that purging alone does not catch, such as the autocorrelation of returns and volatility that can transmit information across the immediate post-test boundary \citep[Section 7.4.2]{lopez_de_prado_2018}.

An empirical leakage audit verifies that the combined purging and embargo machinery leaves no overlap in any split. For each of the 28 splits, the audit counts training observations whose barrier-touch timestamp $t_{1,i}$ falls within the date range of the assigned test group. A non-zero count would indicate that purging missed an overlap. The audit returns zero across all 28 splits, confirming that the pipeline produces leak-free training and test partitions throughout the labelling window. The per-split counts are listed in \autoref{tab:leakage_audit} in the appendix.


\subsubsection{Path matrix}\label{subsubsec:path_matrix}

The 28 splits combine into 7 backtest paths, each of which is a complete out-of-sample sequence covering all 1,168 observations exactly once. The construction rule is the one in \citet[Section 12.4.1]{lopez_de_prado_2018}. For path $p$ and group $g$, the test prediction for $g$ comes from the $p$-th split in canonical order in which $g$ appears as a test group. Each of the eight groups is tested in 7 of the 28 splits, which is why exactly 7 paths are available. Each path therefore uses 4 of the 28 splits, with their test groups together covering all eight groups exactly once.

The path structure converts a single backtest into a distribution. The 7 paths each yield a separate Sharpe ratio, AUC, accuracy, and equity curve for each model. The distribution of these 7 metric values across paths is what the Deflated Sharpe Ratio and the Probability of Backtest Overfitting consume as input (Sections 3.11.3 and 3.11.4), in place of the single-point estimate that a non-combinatorial cross-validation would deliver. A model that performs well on average across paths but with high path-to-path variance is exposed by the distribution analysis in a way that a single-number Sharpe estimate cannot achieve.
...
%   - Per-model z-scoring before averaging. RF and LR produce MDA scores
%     on different absolute scales (RF's permutation drops F1 by
%     approximately 0.005 to 0.05 per feature; LR's drops can be an
%     order of magnitude smaller). Naive averaging of (MDA\_RF +
%     MDA\_LR) / 2 would let RF magnitudes dominate, effectively
%     recovering an RF-only ranking. The implementation z-scores each
%     model's MDA vector across features before averaging, so both
%     models contribute symmetrically to the final ranking on a
%     rank-normalised basis.
%   - Final MDA = mean(zscore(MDA\_RF), zscore(MDA\_LR)) per feature.
...
%
%   Selection rule: keep features with averaged z-MDA > 0; cap at
%   MDA\_TOP\_K\_FRAC = 0.20 (approximately 14 to 15 of 73); minimum
%   floor of 5 features.
%
%   TOP\_K\_FRAC trail (advisor-reviewed):
%   - The cap was tightened from common defaults of 0.40 to 0.50 in
%     successive rounds of methodology development.
%   - The first tightening from 0.40 to 0.30 came after a high-PBO run
%     on the earlier 66-feature pool revealed that only approximately
%     6 features cleared 50\% selection frequency in the stability bar
%     chart, indicating that the long tail of the MDA-ranked feature
%     set was contributing variance rather than signal.
%   - The May 2026 expansion of the feature pool from 66 to 73 columns
%     (the 14-day macro return variants and the AR\_LAGS extension from
%     6 to 10) was an advisor-driven structural change to the feature
%     set; the cap was re-evaluated at the same time using absolute-
%     feature-count arithmetic. At the locked 73-feature pool,
%     0.20 x 73 ~= 14 to 15 features restores the absolute count to
%     roughly the original working point of 0.30 x 66 ~= 20.
%   - The 0.20 cap is the locked working configuration.
%
%   Methodology rationale for the second tightening (0.30 to 0.20):
%   - With approximately 600 events in the model-training partition
%     after the 70/15/15 three-way split (Section 3.9), the KAN and
%     LSTM parameter counts at width1=6, hidden=16 already approach
%     1 sample per parameter at 15 features.
%   - Expanding to 20+ features deepens overparameterisation in the
%     two most-watched neural models, which is why a tighter rather
%     than looser cap was chosen.
%   - Sensitivity sweeps over \{0.10, 0.15, 0.20, 0.25, 0.30\} can be
%     appended as an appendix without modifying the source.
%
%   AR Logistic exception: bypasses MDA entirely; receives pre-MDA
%   matrix and selects 10 lag columns by name.
%
%   Typical result: approximately 14 to 15 features selected per fold
%   from 73 candidates (down from approximately 22 under the previous
%   0.30 cap on the same 73-feature pool).
%
...
%   - 5 seeds, 40 trials per split (was 2 seeds in the previous run; the locked configuration now uses 5 seeds uniformly across neural and classical models).
%
...
%     memorisation).
%   - 5 seeds, 40 trials per split (was 2 seeds in the previous run; uniform 5-seed configuration across all six models in the locked v6 run).
%
...
%   Logistic Regression   | Linear ML   | LR on selected features                | max\_iter=1000              | C, penalty                                                                                          | 5 | 40 | Linear baseline (Q3)
%   Random Forest         | Ensemble    | balanced\_subsample                    | n\_jobs=-1                  | n\_estimators <= 250, max\_depth in [2, 6], min\_samples\_leaf in [15, 40], max\_features            | 5 | 40 | Nonlinear ensemble (Q3)
%   XGBoost               | Ensemble    | 500 trees + early stop@20              | binary:logistic             | max\_depth in [1, 3], lr, min\_child\_weight in [5, 30], subsample, etc. (8)                        | 5 | 40 | Gradient boosting (Q3)
%   LSTM                  | Neural      | window=14, last-hidden, num\_layers=1 | T\_0=25, batch=64           | hidden in {16, 32}, dropout in [0.1, 0.5], lr                                                       | 5 | 40 | Temporal dependencies (Q3)
%   KAN                   | Neural      | [n, width1, 2], single hidden, k=3     | width2=0, T\_0=30, ls=0.1   | width1 in [2, 6], grid in {3, 5}, lr, weight\_decay                                                 | 5 | 40 | Interpretable architecture (Q3, Q4)
...
%
%   - Trial budget:
%     n\_trials=40 per tuned model per split, applied uniformly across
%     LR, RF, XGBoost, LSTM, and KAN. The notebook passes a single
%     n\_trials=40 parameter so every tuned model competes on the same
%     budget; AR Logistic is not tuned. The module-level defaults
%     (N\_TRIALS\_CLASSICAL=60, N\_TRIALS\_NEURAL=40) are reserved for
%     sensitivity experiments outside the headline run; the locked run
%     overrides both to 40 via the pipeline parameter.
%
...
%
% Topics:
%   - Calibration set (70/15/15 three-way chronological split of the
%     training fold): within each outer training fold the data is split
%     chronologically into 70\% model-train, 15\% validation, and 15\%
%     calibration. The validation partition feeds the model's X\_val /
%     y\_val for early stopping and best-state tracking; the calibration
%     partition is held entirely separate and feeds Platt or vector
%     scaling. At N=8 the locked configuration places approximately
%     600 events in the model-training partition and approximately 129
%     events each in the validation and calibration partitions. None of
%     these partitions touches the outer test fold.
%
%   - Why 70/15/15 rather than 80/20 or 80/10/10: the split was widened
%     from an earlier 80/10/10 after a calibration audit revealed all
%     six models systematically under-predicting P(Up) by 4 to 7
%     percentage points; with approximately 750 events per outer
%     training fold, 10\% (approximately 75 events) was insufficient
%     for both the vector-scaling fit (T, b[1] estimated on
%     approximately 75 binary labels) AND the LSTM/KAN early-stopping
%     signal. The 80/20 split that preceded both used a single 20\%
%     subset for both purposes, which introduced subtle leakage between
%     the model's stopping decision and the calibration data; the
%     three-way split removes that dual-use coupling. The current 15\%
%     partitions raise both roles to approximately 129 events at the
%     cost of approximately 75 fewer model-training events per fold
%     (approximately 600 vs approximately 688 under 80/10/10).
%
...
%     model selection.
%
% Weighted vs unweighted calibration (audited, decision: unweighted):
%   - Both fit\_platt\_scaling and fit\_vector\_scaling accept an
%     optional sample\_weight argument that would weight the per-sample
%     NLL by AFML sample weights before averaging. The argument is
%     supported in the calibrator API but intentionally not triggered
%     from pipeline.py.
%   - The rationale is empirical: an audit run on the full six-model
%     pool with weighted Platt + weighted vector scaling enabled showed
%     the weighted variant pushing every model's calibration miss
%     further from the empirical base rate (3 of 6 flagged -> 5 of 6
%     flagged under weighted; calibration deltas grew on every single
%     model relative to the unweighted baseline), and median Sharpes
%     compressed across the cross-section.
%   - Mechanism: AFML weights over-represent the high-weight subset
%     (rare events with little overlapping label structure) whose class
%     balance differs from the population base rate; weighting the
%     calibrator's loss against that subset's distribution tilts the
%     calibrated probability away from the empirical class frequency on
%     the full data.
%   - The unweighted path remains the locked configuration. The
%     weighted code paths are kept available for future experiments
%     without re-implementation cost; a comment block above the
%     calibrator.fit(...) call site in pipeline.py documents the audit
%     evidence so the next reader does not reintroduce the regression.
%
% LSTM logits sliced before calibration:
%   - LSTM produces logits only for windows where all 14 lookback
%     timesteps are non-NaN; the first approximately 14 events of any
%     partition are window-incomplete and would produce NaN logits.
%   - The pipeline slices raw\_logits to valid\_idx BEFORE passing them
%     to the calibrator. An earlier ordering calibrated on the full
%     unsliced logits and sliced afterward, producing a fitted
%     calibrator whose temperature reflected the NaN-padded positions
%     rather than the valid windows; the bug manifested as T
%     consistently hitting the lower bound on LSTM folds and was caught
%     during a calibration audit.
%
...
%      primitives, all smooth everywhere."
%
...
%
% Partial derivatives evaluated at the median of each feature, scaled
% by the feature's standard deviation, give the per-sigma effect on
% the decision function (logit). All three surviving features now have
% finite, well-defined gradients at the dataset median (the previous
% run's natgas Abs-primitive non-differentiability is gone). Locked-run
% table (paste as a tabular environment in the thesis):
%
%   Feature           | median  | std    | d/dx at median | sigma-effect on logit | sigma Delta\_p (linearised)
%   eth\_btc\_ratio    | +0.0460 | +0.0247| -41.1765       | -0.2805               | -0.0598
%   log\_returns\_lag6 | -0.0008 | +0.0394| +30.6748       | -0.4608               | -0.0983
%   natgas\_ret\_30    | +0.0025 | +0.1618| +0.4236        | +0.0838               | +0.0179
%
%   - Reading the sensitivity table. log\_returns\_lag6 carries the
%     largest per-sigma effect on the logit (-0.4608) and the largest
%     per-sigma linearised probability shift (-0.0983), driven by the
%     quartic primitive in the first outer term. eth\_btc\_ratio is
%     second (-0.2805 / -0.0598) and contributes via the cos inner
%     argument and the tanh outer wrapper. natgas\_ret\_30 carries the
%     smallest per-sigma effect (+0.0838 / +0.0179) despite appearing
%     in all four outer terms; the bounded tanh and x\^3 primitives
%     saturate near the dataset median, attenuating its marginal
%     contribution. The qualitative pattern from the symbolic form:
%     natgas\_ret\_30 enters via tanh and x\^3 (both bounded primitives
%     at the dataset median), log\_returns\_lag6 enters via x\^4 and
%     the trigonometric inner arguments (x\^4 contributes a steep
%     gradient if the inner argument is far from zero; at the dataset
%     median log\_returns\_lag6 ~ -0.0008 the x\^4 term contributes a
%     small gradient), eth\_btc\_ratio enters via the cos inner
%     argument and the tanh outer wrapper (both bounded and smooth at
%     the dataset median). The locked formula is everywhere-
%     differentiable; the non-differentiability that previous-run
%     readers would have had to manage is resolved.
...
%      feature posts an undefined gradient anywhere. RQ4
...
%   - Numerical sensitivity at the dataset median. All three surviving
%     features have finite, well-defined gradients at the dataset
%     median (the previous run's natgas Abs-primitive non-
%     differentiability is gone). From the locked-notebook sensitivity
%     table (Section 4.6): log\_returns\_lag6 carries the largest
%     per-sigma effect (sigma-effect on logit -0.4608, sigma Delta\_p
%     -0.0983), driven by the x\^4 primitive in the first outer term;
%     eth\_btc\_ratio is second (sigma-effect -0.2805, sigma Delta\_p
%     -0.0598), entering via the cos inner argument and the tanh outer
%     wrapper; natgas\_ret\_30 carries the smallest per-sigma effect
%     (sigma-effect +0.0838, sigma Delta\_p +0.0179), with the bounded
%     tanh and x\^3 primitives saturating near the dataset median
%     despite the feature appearing in all four outer terms. The
%     qualitative pattern from the symbolic form: natgas\_ret\_30
%     enters via tanh and x\^3 (both bounded at the dataset median),
%     log\_returns\_lag6 enters via x\^4 and the trigonometric inner
%     arguments (x\^4 contributes a steep gradient if the inner
%     argument is far from zero; at the dataset median
%     log\_returns\_lag6 ~ -0.0008 the x\^4 term contributes a small
%     gradient), eth\_btc\_ratio enters via the cos inner argument and
%     the tanh outer wrapper (both bounded and smooth at the dataset
%     median). The locked formula is everywhere-differentiable; the
%     non-differentiability that previous-run readers would have had
%     to manage is resolved.
...
%         formula. Sensitivity at the dataset median is now finite for
...
%     from 3 seeds / 504 entries in the previous run), 40 Optuna
...
% a hedge. Target <= 10\% of the textual part (~3.5 pages including
% the future-work block).
%
% Suggested five-block structure:
%
...
%   - One paragraph (or compact bullet list) on future work.
%     Cochrane: do not write your grant application here. Concrete,
%     actionable directions only.
%       - MultKAN (KAN 2.0) on the last fold. Multiplication nodes
%         enable discovery of multiplicative interactions. Same
%         symbolic pipeline. Cite: \cite{liu_kan2_2024}.
%       - Higher-frequency data. Hourly bars would multiply CUSUM
%         events approximately 24 times and unlock microstructure
%         features. Pipeline timeframe-agnostic.
%       - Per-fold symbolic extraction. Count which features and
%         primitives recur across all 28 folds to distinguish
%         structural signal from one-off noise.
...
%         classifier's output, trained to predict whether to take the
%         trade (AFML Ch. 3). Singh and Joubert (2019) provide
%         empirical evidence that meta-labeling improves signal
%         efficacy across asset classes; applying it on top of this
%         thesis's calibrated probabilities is the natural next layer.
%         Cite: \cite{singh_joubert_2019}.
%       - Alternative assets. ETH, gold for symbolic-formula
%         comparison (different on-chain availability).
%       - Walk-forward vs. CPCV. Compare conclusions under both
%         protocols.
...
\caption{Full feature list (73 features) grouped by family.}
...
\caption{Eight CPCV groups overlaid on BTC closing price, each holding 146 of the 1,168 aligned observations.}
...
\bottomrule
\end{tabular}
\caption{Per-split leakage audit for the 28 CPCV splits. Train and test counts are after purging and embargo. Leaks count training observations whose barrier-touch timestamp falls within the assigned test group's calendar range, with zero indicating a clean split.}
